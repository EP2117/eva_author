...
function pay_mode(val){
	$("#ref_details,#aeA_chequeno,#aeA_chequedate").val('');
	if(val==1){
...
function getEmployeesName(val,obj){	
 ...
 function GetCurrecy_amt(){
	 ...
	 var cur_date	= $("#expense_date").val();
 	$.get("currency-amt.php",
		  {cur_id:cur_id,cur_amt:cur_amt,cur_date:cur_date},
		  function(data){
			  var data_v 	= data.split('@'); //alert(data_v[0]);
 ...
 function GetDebCur_amt(){
	...
	var cur_date	= $("#expense_date").val();
 	$.get("currency-amt.php",
		  {cur_id:cur_id,cur_amt:cur_amt,cur_date:cur_date,type:"debit"},
		  function(data){
