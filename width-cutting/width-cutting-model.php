<?php
	...
	function insertQuotation(){
		...
		$width_cutting_brand_id      						= trim($_POST['width_cutting_brand_id']);
		//Product Detail
		...
		$width_cutting_product_detail_osf_uom_ton			= $_POST['width_cutting_product_detail_osf_uom_ton'];
		...
		$insert_width_cutting 							= sprintf("INSERT INTO width_cutting(width_cutting_uniq_id, width_cutting_date,
																						...
																						width_cutting_sw_check,width_cutting_brand_id) 
																	VALUES 	 		 (	'%s', '%s', 
																					  	'%d', 
																					  	'%d', '%s',
																					  	'%d', '%d', 
		...
		// purchase order pproduct details
			...
			for($i = 0; $i < count($width_cutting_width_detail_name); $i++) {
				...
				$brand_id											= 	$width_cutting_product_detail_product_brand_id;
				...
				$total_ton											=   $width_cutting_product_detail_osf_uom_ton;		
				...
				stockLedger($mother_child_type,'out',$width_cutting_id,$width_detail_id,$produt_id,$length_feet,$length_meter,$ton_qty,$kg_qty,$product_detail_qty, $width_cutting_branch_id,  $product_con_entry_godown_id, $width_cutting_date, $width_cutting_no, $stock_ledger_entry_type, '2',$width_inches,$width_mm,$product_colour_id,$thick);
			...
			if($detail_request_fields) {
				...
				stockLedger($mother_child_type,'in',$width_cutting_id,$product_con_entry_detail_id,$produt_id,$length_feet,$length_meter,$ton_qty,$kg_qty,$product_detail_qty, $width_cutting_branch_id,  $product_con_entry_godown_id, $width_cutting_date, $width_cutting_no,$stock_ledger_entry_type, '2',$width_inches,$width_mm,$product_colour_id,$product_thick);
	...
	function listQuotation(){
		...
		if(!empty($_REQUEST['search_branch_id'])){
			$where	.=" AND width_cutting_branch_id = '".$_REQUEST['search_branch_id']."'";
		}
		...
		{
		$where.="AND width_cutting_date BETWEEN '".NdateDatabaseFormat($_REQUEST['search_from_date'])."'
		 ...
		 $select_width_cutting		=	"SELECT 
											...
											product_con_entry_child_product_detail_name,
											brand_name
										 FROM  width_cutting_product_details
										 LEFT JOIN 
											width_cutting ON width_cutting_id= width_cutting_product_detail_width_cutting_id
										LEFT JOIN 
											product_con_entry_child_product_details 
														ON 
											product_con_entry_child_product_detail_id= width_cutting_product_detail_product_id	
										LEFT JOIN 
											 brands ON brand_id= product_con_entry_child_product_detail_product_brand_id		
										 WHERE 
											width_cutting_deleted_status 	= 	0 	 $where
										 GROUP BY width_cutting_product_detail_width_cutting_id
										 ORDER BY  
	...
	function editQuotation(){
		...
		$select_width_cutting		=	"SELECT 
												...
												width_cutting_no,width_cutting_brand_id,
												width_cutting_branch_id,width_cutting_id,
												width_cutting_sw_check, godown_name, brand_name
											 FROM 
												width_cutting 
												LEFT JOIN godowns ON width_cutting_godown_id = godown_id
												LEFT JOIN brands ON width_cutting_brand_id = brand_id
											 WHERE 
												width_cutting_deleted_status 	=  0 							AND 
	...
	function updateQuotation(){
		...
		$width_cutting_product_detail_product_id     		= $_POST['width_cutting_product_detail_product_id'];
		$width_cutting_product_detail_product_thick_ness    = $_POST['width_cutting_product_detail_product_thick_ness'];
		$width_cutting_product_detail_product_brand_id     	= $_POST['width_cutting_product_detail_product_brand_id'];
		$width_cutting_product_detail_product_colour_id     	= $_POST['width_cutting_product_detail_product_colour_id'];
		...
		$i					= 0;
			...
			if($detail_request_fields) {
					...
					$product_detail_width_inches	= $width_cutting_product_detail_width_inches[$i];
					$product_detail_width_mm		= $width_cutting_product_detail_width_mm[$i];
					$product_detail_product_thick_ness	= $width_cutting_product_detail_product_thick_ness[$i];
		...
		for($i = 0; $i < count($width_cutting_width_detail_name); $i++) {
				...
				$width_val											= $product_detail_width_inches; 
				$sales_width_val									= ($width_cutting_width_detail_width_inches_one[$i]+$width_cutting_width_detail_width_inches_two[$i]+
																	  $width_cutting_width_detail_width_inches_three[$i]+$width_cutting_width_detail_width_inches_four[$i]);
				$sales_length_val									= $width_cutting_width_detail_length[$i]*$width_cutting_width_detail_length_qty[$i];	
				$sales_width_val;		 
				...
				$product_cal										=  explode("@",$prd_calc_val);
				$produt_id											=  $width_cutting_product_detail_product_id[$i];
				$length_feet										=  $total_length;
				$length_meter										= 	$product_cal[3];
				$get_raw_val										=	Child_prod_detail($produt_id);
				$brand_id											= 	$width_cutting_product_detail_product_brand_id[$i];
				
				$thick												= 	$product_detail_product_thick_ness;
				$width_inches										= 	$product_detail_width_inches;
				$width_mm											= 	$product_detail_width_mm;
				$product_colour_id									= 	$width_cutting_product_detail_product_colour_id[$i]; 
				$total_ton											=  	GetTotallenWei($brand_id,$thick,$width_inches,'');
				$ton_qty											= 	$width_cutting_product_detail_osf_uom_ton[$i]*$length_feet;
				$kg_qty												= 	$ton_qty*1000;
				$product_detail_qty									= 	"-1";
				$stock_ledger_entry_type							= 	"width-cutting-mother";
				$product_con_entry_godown_id						= 	$width_cutting_godown_id;
		...
		for($i=0;$i<count($product_con_entry_child_product_detail_code);$i++){
				...
				if(isset($product_con_entry_child_product_detail_id[$i])){
					$update_product_con_entry_child_product_detail = sprintf("	UPDATE 
																		...
																		SET  
																			...
																			product_con_entry_child_product_detail_total  					= '%f',
																			product_con_entry_child_product_detail_ton_qty  				= '%f',
																			product_con_entry_child_product_detail_kg_qty  					= '%f',
																			product_con_entry_osf_length_feet  								= '%f',
																			product_con_entry_osf_length_m  								= '%f',
																			product_con_entry_osf_uom_ton  									= '%f',
																			product_con_entry_osf_uom_kg  									= '%f',
				...
				$insert_product_con_entry_product_detail 		= sprintf("INSERT INTO product_con_entry_child_product_details 
																	...
																	VALUES     	('%s', '%d',
																				...
																				$product_con_entry_child_product_detail_product_brand_id[$i],
																				$product_con_entry_child_product_detail_product_category_id[$i],
																				$ton_qty,$kg_qty,$osf_feet,$osf_feet_mm,$osf_ton,$osf_ton_kg,'width-cutting');
	...
	function deleteInventoryrequest(){
		...
		deleteUniqRecords('width_cutting', 'width_cutting_deleted_by', 'width_cutting_deleted_on' , 'width_cutting_deleted_ip','width_cutting_deleted_status', 'width_cutting_id', 'width_cutting_uniq_id', '1');
				...
				{ 
					...
					$width_cutting_id 			= getId('width_cutting', 'width_cutting_id', 'width_cutting_uniq_id',$deleteCheck); 
					 ...
					 while($record_prn_detal = mysql_fetch_array($result_prn_detal)){
					     ...
					     $update_cs_detail 	= "UPDATE  stock_ledger
												...
												WHERE               
														...
														stock_ledger_entry_type             	= 'width-cutting-mother' 										AND
														stock_ledger_entry_id					= '".$width_cutting_id."'										AND
														stock_ledger_entry_detail_id			= '".$record_prn_detal['width_cutting_width_detail_id']."'";
					 ...
					 while($resultChData = mysql_fetch_array($result_grn_ch_detal)){
					     ...
					     $update_cs_detail 	= "UPDATE  stock_ledger
													SET 
														...
														stock_ledger_deleted_ip    				= '".$ip."'
												WHERE               
														stock_ledger_entry_type             	= 'width-cutting-child' 											AND
														stock_ledger_entry_id					= '".$width_cutting_id."'											AND
