	...
	$select_invoice_entry		=	"SELECT 
									...
									LEFT JOIN 
											(SELECT 
												...
												SUM(delivery_customer_product_detail_qty) AS del_qty 
											FROM 
												delivery_customer_product_details  	
											WHERE 
												delivery_customer_product_detail_deleted_status 				= 0  					
											GROUP BY 
												delivery_customer_product_detail_invoice_entry_id) del_table 
									ON 
											delivery_customer_product_detail_invoice_entry_id 	= invoice_entry_id
									 WHERE 
										...
										invoice_entry_customer_id											= 	'".$customer_id."'
...
?>
		...
		$select_invoice_no = "SELECT delivery_customer_no FROM delivery_customer 
								  WHERE delivery_customer_deleted_status =0
								  AND delivery_customer_financial_year = '".$_SESSION[SESS.'_session_financial_year']."'
								  AND delivery_customer_invoice_entry_id	= '".$record_invoice_entry['invoice_entry_id']."'
		...
		?>
<?php if($record_invoice_entry['invoice_entry_type_id']==4){ $type_id=1;}else if($record_invoice_entry['invoice_entry_type_id']==2 || $record_invoice_entry['invoice_entry_type_id']==1){$type_id=2; }else{$type_id=3;}
?>
					...
					<tr class="odd gradeX">
						...
						<input type="hidden" name="invoice_entry_type_id" id="invoice_entry_type_id<?php echo $record_invoice_entry['invoice_entry_id'];?>" value="<?=$type_id?>" >
