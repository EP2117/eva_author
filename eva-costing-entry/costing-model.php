<?php 
	...
	$where	= '';
		if(!empty($_REQUEST['search_branch_id'])){
			$where	.=" AND cs_branchid = '".$_REQUEST['search_branch_id']."'";
		}
		...
		{
		$where.="AND cs_costingdate BETWEEN '".NdateDatabaseFormat($_REQUEST['search_from_date'])."'
		...
		$query  = "SELECT costingId,branch_name,cs_port_location,cs_port_name,DATE_FORMAT(cs_costingdate ,'%d/%m/%Y') AS cs_costingdate
					...
					WHERE cs_deleted_status =0 $where	
	...
	function editCosting($id){
		$query  = "SELECT *,DATE_FORMAT(cs_costingdate ,'%d/%m/%Y') AS cs_costingdate
				    FROM costing_entry 
					LEFT JOIN purchase_invoice ON invoiceId = cs_invoiceId
					WHERE costingId ='$id'";
	...
	function editRequestproduct($id){
		 ...
		 $query = "SELECT *,currency_name,pur_costing_name
					FROM costing_entry_details 
					LEFT JOIN currencies ON currency_id = c_currencyId
					LEFT JOIN pur_costings ON pur_costing_id = c_costingname
