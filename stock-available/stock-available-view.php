											...
											<div class="form-group">
												...
												<option value="">-Select-</option>
												 <option value="2"  <?php if(searchValue('product_status')=="2" ){ ?> selected="selected" <?php } ?>> Raw</option>
												  <option value="3"  <?php if(searchValue('product_status')=="3" ){ ?> selected="selected" <?php } ?>> Finished </option>
								...
								<table id="stock_table"  class="table table-striped table-bordered table-hover" >
									...
									<?php $amt=0;
										...
										<tr class="odd gradeX">
											...
											<?php
												$warr = searchValue('warehouseid');
												$ton_per_ft = 1;
												//get ton per feet depends on thickness and width inches
												$ton_per_ft_sql = "SELECT * FROM ton_per_feet WHERE thick='".$result['stock_ledger_thick_ness']."' AND inches='".number_format($result['product_con_entry_child_product_detail_width_inches'],2)."' AND deleted_status = 0";
												$ton_per_ft_qry = mysql_query($ton_per_ft_sql);
												while($ton_per_ft_data = mysql_fetch_assoc($ton_per_ft_qry)) {
													$ton_per_ft = $ton_per_ft_data['ton'];
												}
												
											?>
											
											<?php
												if($result['sal_length_feet'] == 0) {
													if(count($warr) == 1 && $warr[0] == 2) {
													//calulate feet depends on ton
													$feet = $result['sal_weight_tone']/$ton_per_ft;
													//$feet = ($result['sal_weight_tone']/$result['pur_weight_tone']) * $result['pur_length_feet'];
													} else {
														$feet = $result['sal_length_feet'];
													}
											?>
											<td><?php echo number_format($feet,2,'.',''); ?></td>
											<td><?php echo number_format($feet*0.3048,2,'.',''); ?></td>
											<?php
												} else {
													if(count($warr) == 1 && $warr[0] == 2) {
														//check ton exist or not
														if($result['sal_weight_tone'] == 0) {
															$feet = $result['sal_length_feet'];
														}
														else {
															//calculate feet depends on ton
															$feet = $result['sal_weight_tone']/$ton_per_ft;
															$feet = $feet + $result['sal_length_feet'];
											...
											<?php
												if($result['sal_weight_tone'] == 0) {
													if(count($warr) == 1 && $warr[0] == 2) {
														//calculate ton depends on feet
														$ton = $result['sal_length_feet'] * $ton_per_ft;
														//$ton = ($result['sal_length_feet']/$result['pur_length_feet']) * $result['pur_weight_tone'];
														$kg = $ton * 1000;
													} else {
														$ton = $result['sal_weight_tone'];
														$kg = $ton * 1000;
													}
											?>
											<td><?php echo number_format($ton,4,'.',''); ?></td>
											<td><?php echo number_format($result['sal_weight_kg'],2,'.',''); ?></td>
											<?php
												} else {
													if(count($warr) == 1 && $warr[0] == 2) {
														//check feet exist or not
														if($result['sal_length_feet'] == 0) {
															$ton = $result['sal_weight_tone'];
															$kg = $ton * 1000;
														} else {
															...
															$ton = $ton + $result['sal_weight_tone'];
															//$kg  = $kg + $result['sal_weight_tone'];
															$kg = $ton * 1000;
														}
													} else {
														$ton = $result['sal_weight_tone'];	
											...
											<td><?php echo number_format($result['closing_weight_tone'],4,'.',''); ?></td>
											<td><?php echo number_format($result['closing_weight_kg'],2,'.',''); ?></td>-->
											
											<td><?php echo number_format($result['pur_length_feet'] + $feet,2,'.',''); ?></td>
											<td><?php echo number_format(($result['pur_length_feet'] + $feet)*0.3048,2,'.',''); ?></td>
											<td><?php echo number_format($result['pur_weight_tone'] + $ton,4,'.',''); ?></td>
											<td><?php echo number_format($result['pur_weight_kg'] + $kg,2,'.',''); ?></td>
										
										</tr>
																   
									 <?php
									$amt=$amt+($result['pur_length_feet'] + $feet); 
									$total_ton=$total_ton+($result['pur_weight_tone'] + $ton);	}
										}
									  ?>
									</tbody>
                                     <tfoot>
									  ...
									  ?>
										<tr >
											...
											<td ><b><?php echo number_format($total_ton,4,'.','');?></b></td><td ></td>
										</tr>
                                        </tfoot>
							...
							<table  class="table table-striped table-bordered table-hover" id="stock_table">
									...
									<?php
										...
										<tr class="odd gradeX">
											...
											<td><?php echo number_format($result['stock_ledger_weight_tone'],4,'.',''); ?></td>
							...
							<table id="dataTables-example"  class="table table-striped table-bordered table-hover" style="width:100%">
									...
									<?php
										...
										<tr class="odd gradeX">
											...
											<?php
												if($result['product_is_opp'] == '1') {
											?>
											<td><?php echo $result['closing_feet'] == 0  ? '0' : number_format($result['closing_qty'],2,'.',''); ?></td>
											<?php
												}
												else {
											?>
											...
											<!-- Added by AuthorsMM -->
											<td>
											<?php echo $result['stock_ledger_product_feet_per_quantity']; ?>
											</td>
											<td>
											<?php
												$qty = $result['closing_qty']==""?0:$result['closing_qty'];
												//$total_feet = $qty * $result['stock_ledger_product_feet_per_quantity'];
												$total_feet = $result['closing_feet'];
												echo $total_feet;
											?>
											</td>
											<!-- End -->
