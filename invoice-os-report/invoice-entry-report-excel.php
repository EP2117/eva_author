...
$total_amt		= 0;
	foreach($results as $value) {
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('E'.$row, $value['invoice_entry_net_amount'])
					->setCellValue('F'.$row, $value['received_amt'])
					->setCellValue('G'.$row, $value['invoice_entry_net_amount']-$value['received_amt']);
		...
		$total_amt 		+= $value['invoice_entry_net_amount'];
		$received_amt 	+= $value['received_amt'];
		$balance_amt 	+= $value['invoice_entry_net_amount']-$value['received_amt'];
		$objPHPExcel->getActiveSheet()->getStyle('E'.$row.":".'G'.$row)->getNumberFormat()->setFormatCode('0.00');
