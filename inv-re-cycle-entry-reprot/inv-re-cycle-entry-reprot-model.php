<?php  
	...
	{
		$where = " WHERE recycle_entry_deleted_status =0  AND recycle_entry_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_entry_no']) && (!empty($_REQUEST['search_entry_no']))) {
			$where .= " AND recycle_entry_no = '".dataValidation($_REQUEST['search_entry_no'])."'";
		}
		
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND recycle_entry_branch_id = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		...
		if(isset($_REQUEST['search_product_id']) && (!empty($_REQUEST['search_product_id']))) {
			$where .= " AND recycle_entry_product_detail_product_id = '".dataValidation($_REQUEST['search_product_id'])."'";
		}
		...
		if(isset($_REQUEST['search_godown_id']) && (!empty($_REQUEST['search_godown_id']))) {
			$where .= " AND recycle_entry_godown_id = '".dataValidation($_REQUEST['search_godown_id'])."'";
		}
		
		
		
		                    $select_invoice_entry = "SELECT recycle_entry_id,recycle_entry_uniq_id,recycle_entry_no, 
								  	recycle_entry_date,recycle_entry_godown_id, 
								 	product_con_entry_child_product_detail_name as product_name,
									product_uom_name,
									product_con_entry_child_product_detail_code as product_code,
									product_colour_name,brand_name,
								  
								  product_con_entry_child_product_detail_thick_ness,product_con_entry_child_product_detail_mas_product_id,
								  product_con_entry_child_product_detail_width_inches,product_con_entry_child_product_detail_width_mm,
								  product_con_entry_child_product_detail_length_feet,product_con_entry_child_product_detail_length_mm	,
								  ...
								  LEFT JOIN recycle_entry ON recycle_entry_id = product_con_entry_child_product_detail_product_con_entry_id
								...
								ON 
								  ...
								  $where
								  ORDER BY product_con_entry_child_product_detail_mas_product_id,recycle_entry_date ASC";
