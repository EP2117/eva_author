<?php  
	...
	{
		$where = " WHERE production_entry_product_detail_deleted_status =0 AND production_entry_deleted_status = 0 AND production_entry_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_entry_no']) && (!empty($_REQUEST['search_entry_no']))) {
			$where .= " AND production_entry_no = '".dataValidation($_REQUEST['search_entry_no'])."'";
		}
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND production_entry_branch_id = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		if(isset($_REQUEST['search_supplier_id']) && (!empty($_REQUEST['search_supplier_id']))) {
			$where .= " AND production_entry_production_section_id = '".dataValidation($_REQUEST['search_supplier_id'])."'";
		}
		if(isset($_REQUEST['search_color_id']) && (!empty($_REQUEST['search_color_id']))) {
			$where .= " AND production_entry_product_detail_product_colour_id = '".dataValidation($_REQUEST['search_color_id'])."'";
		}
		if(isset($_REQUEST['search_thick_id']) && (!empty($_REQUEST['search_thick_id']))) {
			$where .= " AND production_entry_product_detail_product_thick = '".dataValidation($_REQUEST['search_thick_id'])."'";
		}
		if(isset($_REQUEST['search_prodcut_id']) && (!empty($_REQUEST['search_prodcut_id']))) {
			$where .= " AND production_entry_product_detail_product_id = '".dataValidation($_REQUEST['search_prodcut_id'])."'";
		}
		if(isset($_REQUEST['search_product_type']) && (!empty($_REQUEST['search_product_type']))) {
			$where .= " AND production_entry_type_id = '".dataValidation($_REQUEST['search_product_type'])."'";
		}        
		 $select_invoice_entry = "SELECT 
										...
										production_entry_product_detail_s_width_inches,
										production_entry_product_detail_s_width_mm,
										production_entry_product_detail_sl_feet,
										production_entry_product_detail_sl_feet_mm,
										production_entry_product_detail_s_weight_inches,
										production_entry_product_detail_s_weight_mm,
										production_entry_product_detail_qty,
										product_code,
										product_name,
										grn_entry_no,production_entry_type_id,
										grn_entry_date
								  FROM  production_entry_product_details
								  LEFT JOIN production_entry ON production_entry_id = production_entry_product_detail_production_entry_id
								  LEFT JOIN grn_entry ON grn_entry_id = production_entry_product_detail_production_entry_id
								  LEFT JOIN 
								  		products 
								  ON 
								  ...
								  LEFT JOIN product_colours ON product_colour_id =  production_entry_product_detail_product_colour_id
								  $where
								  ORDER BY production_entry_no ASC";
