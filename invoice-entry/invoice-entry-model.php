<?php
	function GetAutoNo($quotation_id){
	
		  $select_invoice_no = "SELECT MAX(SUBSTRING_INDEX(invoice_entry_no,'-',-1)) AS maxval FROM invoice_entry 
								  WHERE invoice_entry_deleted_status =0
								  AND invoice_entry_direct_type =2
								  
								  AND invoice_entry_quotation_entry_id ='".$quotation_id."'
		...
		$maxval = $record_invoice_no['maxval']; //echo $maxval;exit;

		/*if($maxval > 0) {

			$invoice_entry_no = substr(('00000'.++$maxval),-1);
	...
	$select_branch	= "SELECT branch_prefix FROM branches WHERE  branch_id = '".$_POST['invoice_entry_branch_id']."' ";
		...
		$invoice_entry_no                  				= trim($_POST['invoice_entry_no']).'-'.GetAutoNo($_POST['invoice_entry_quotation_entry_id']); 
...
$invoice_entry_product_detail_s_weight_met   		= isset($_POST['invoice_entry_product_detail_s_weight_met'])?$_POST['invoice_entry_product_detail_s_weight_met']:'';
...
$invoice_entry_product_detail_brand_id				= $_POST['invoice_entry_product_detail_brand_id'];	
		...
		$insert_invoice_entry 							= sprintf("INSERT INTO invoice_entry  (invoice_entry_uniq_id, invoice_entry_date,
																			...
																			VALUES 	 		 ('%s', '%s', 
																							 ...
																							 $invoice_entry_tax_status,$invoice_entry_total_amount);
			...
			if($detail_request_fields) {
				...
				$insert_invoice_entry_product_detail 		= sprintf("INSERT INTO invoice_entry_product_details 
																	...
																	VALUES     ('%s', '%d', 
																		 ...
																		 $invoice_entry_product_detail_quotation_detail_id[$i],
				...
				//if($invoice_entry_type==1 && $_SESSION[SESS.'_session_user_branch_type']==1){
					...
					//echo  $invoice_entry_prd_type;exit;
					...
					$select_det="SELECT sum(stock_ledger_weight_tone) as qty,stock_ledger_product_id,stock_ledger_mother_child_type as mother_child_type  FROM stock_ledger
								...
								AND stock_ledger_colour_id='".$invoice_entry_product_detail_color_id[$i]."'
								AND product_brand_id='".$invoice_entry_product_detail_brand_id[$i]."'
					...
					$select_det1="SELECT sum(stock_ledger_weight_tone) as qty,stock_ledger_product_id,stock_ledger_mother_child_type as mother_child_type   FROM stock_ledger
								LEFT JOIN product_con_entry_child_product_details ON product_con_entry_child_product_detail_id =stock_ledger_product_id
					 			WHERE stock_ledger_thick_ness ='".$invoice_entry_product_detail_product_thick[$i]."' 
								AND stock_ledger_colour_id='".$invoice_entry_product_detail_color_id[$i]."'
								AND product_con_entry_child_product_detail_product_brand_id='".$invoice_entry_product_detail_brand_id[$i]."'
								AND stock_ledger_mother_child_type=2 AND stock_ledger_godown_id=2 AND stock_ledger_prd_type=2
								GROUP BY stock_ledger_product_id,stock_ledger_mother_child_type,stock_ledger_branch_id HAVING qty>0 ";
					//echo $select_det1;exit;
	...
	stockLedger($child_type,'out',$invoice_entry_id,$detail_id,$produt_id,$length_feet,$length_meter,$ton_qty,$kg_qty,$produt_qty, $invoice_entry_branch_id,  $godown_id, $invoice_entry_date, $invoice_entry_no,$entry_type,'2',$width_inches,$width_mm,$product_colour_id,$product_thick);
		...
		$invoice_entry_net_amount = $invoice_entry_total_amount;
		...
		//echo $acc_amount.'-'.$invoice_entry_net_amount;exit;
		...
		pageRedirection("invoice-entry/index.php?page=add&msg=1&print_status=1&inv_id=$invoice_entry_uniq_id");
			...
			//	echo $update_invoice_entry_product_detail; exit;
				...
				$invoice_entry_product_detail_uniq_id 	= generateUniqId();
				 $insert_invoice_entry_product_detail 		= sprintf("INSERT INTO invoice_entry_product_details 
																				...
																				invoice_entry_product_detail_invoice_entry_id,
																				 ...
																				 invoice_entry_product_detail_color_id,
																				 invoice_entry_product_detail_entry_type,
																				 invoice_entry_product_detail_s_weight_met,invoice_entry_product_detail_mother_child_type) 
																	VALUES     ('%s', '%d', 
																				'%d', 
																				'%d', '%d', 
																				'%f', '%f',
																		 ...
																		 $invoice_entry_quotation_entry_id,$invoice_entry_product_detail_color_id[$i],
																		 $invoice_entry_product_detail_entry_type[$i],
																		 $invoice_entry_product_detail_s_weight_met[$i],$invoice_entry_product_detail_mother_child_type[$i]);
																		// echo $insert_invoice_entry_product_detail; exit;
				mysql_query($insert_invoice_entry_product_detail);
				$detail_id	= mysql_insert_id();
				}
				//if($invoice_entry_type==1 && $_SESSION[SESS.'_session_user_branch_type']==1){
					if($invoice_entry_product_detail_entry_type[$i]=='4'){
								...
								$godown_id										= 	"2";
								$produt_id										=	$invoice_entry_product_detail_product_id[$i];
								$mother_child_type								=	$invoice_entry_product_detail_mother_child_type[$i];
								$produt_qty										=	(-1*$invoice_entry_product_detail_qty[$i]);
							stockLedger($mother_child_type,'out',$invoice_entry_id,$detail_id,$produt_id,$length_feet,$length_meter,$ton_qty,$kg_qty,$produt_qty, $invoice_entry_branch_id,  $godown_id, $invoice_entry_date, $invoice_entry_no,$stock_ledger_entry_type,'1');
					}
					else{
	...
	function deleteInventoryrequest(){
		...
		{ 
			...
			$select_grn_ch_detal			= "SELECT
			 ...
			 //Acc Transaction
			 DeleteAccountTrasaction($invoice_entry_id, 'Invoice');
			 DeleteAccountTrasaction($invoice_entry_id, 'advance');
		}
