...
function AddSodetail(){
	...
	for (var i = 0; i < document.getElementById('so_list_form').elements.length; i++) {
		...
		if (document.getElementById('so_list_form').elements[i].checked ==true) {
			...
			//var invoice_no					= quotation_entry_no+"-"+inv_cnt;// alert(invoice_no);
			var invoice_no					= quotation_entry_no;
			...
			var row_cnt     				= parseFloat(table.rows.length)-1;	
				...
				document.getElementById('invoice_entry_advance_amount').value	= Math.round(advance_amount);
				document.getElementById('invoice_entry_gross_amount').value	= Math.round(gross_amount);
				document.getElementById('invoice_entry_transport_amount').value	= Math.round(transport_amount);
    				document.getElementById('invoice_entry_tax_per').value	= Math.round(tax_per);
				document.getElementById('invoice_entry_tax_amount').value	= Math.round(tax_amount);
				document.getElementById('invoice_entry_net_amount').value	= Math.round(net_amount);
...
function AddproductDetail(){
	...
	for (var i = 0; i < x.length; i++) {
		if (x[i].checked ==true) {
			...
			var detail_id 					= document.getElementById('detail_id'+ord_id).value;
			...
			var product_s_weight_met		= document.getElementById('product_s_weight_met'+ord_id).value;//alert(product_s_weight_met); 
			...
			apnd	+= '<tr><td><input type="hidden"  name="invoice_entry_product_detail_mother_child_type[]" id="invoice_entry_product_detail_mother_child_type'+row_cnt+'" value="'+mother_child_type+'" />'+product_brand_name+'</td><td>'+product_name+'<input type="hidden"  name="invoice_entry_product_detail_product_id[]" id="invoice_entry_product_detail_product_id'+detail_id+'" value="'+product_id+'" /><input type="hidden"  name="invoice_entry_product_detail_product_type[]" id="invoice_entry_product_detail_product_type" value="'+product_type+'" /><input type="hidden"  name="invoice_entry_product_detail_quotation_detail_id[]" id="invoice_entry_product_detail_quotation_detail_id" value="'+detail_id+'" /><input type="hidden"  name="invoice_entry_product_detail_entry_type[]" id="invoice_entry_product_detail_entry_type" value="'+t_id+'" /><input type="hidden"  name="invoice_entry_product_detail_id[]" id="invoice_entry_product_detail_id'+row_cnt+'" value="" /><input type="hidden"  name="invoice_entry_product_detail_brand_id[]" id="invoice_entry_product_detail_brand_id'+row_cnt+'" value="'+product_brand_id+'" /></td>';
			...
			apnd	+= '<td><input class="form-control" type="text"  name="invoice_entry_product_detail_s_weight_inches[]" id="invoice_entry_product_detail_s_weight_inches'+row_cnt+'"  onblur="GetLcalFeet(9,'+row_cnt+');GetLcalc(3,'+row_cnt+');RawdiscountPerFind('+row_cnt+')"   value="'+product_s_weight_inches+'"   /></td> <td><input class="form-control" type="text"  name="invoice_entry_product_detail_s_weight_mm[]" id="invoice_entry_product_detail_s_weight_mm'+row_cnt+'"  onblur="GetLcalc(4,'+row_cnt+')"  value="'+product_s_weight_mm+'"   /><input type="hidden"  name="invoice_entry_product_detail_qty[]" id="invoice_entry_product_detail_qty'+row_cnt+'"  value="" /></td><td><input class="form-control" type="text"  name="invoice_entry_product_detail_s_weight_met[]" id="invoice_entry_product_detail_s_weight_met'+row_cnt+'"  onblur="GetLcalc(4,'+row_cnt+')"  value="'+product_s_weight_met+'"  /></td>';
			
			apnd	+= ' </td><td><input class="form-control" type="text"  name="invoice_entry_product_detail_rate[]" id="invoice_entry_product_detail_rate'+row_cnt+'"  value="'+product_rate+'"   onBlur="RawdiscountPerFind('+row_cnt+')"    /></td><td><input class="form-control" type="text"  name="invoice_entry_product_detail_total[]" id="invoice_entry_product_detail_total'+row_cnt+'" value="'+product_total_amt+'" /><input class="form-control" type="hidden"  name="invoice_entry_product_detail_sl_feet_mm[]" id="invoice_entry_product_detail_sl_feet_mm'+row_cnt+'" value="'+product_sl_feet_mm+'"  /><input class="form-control" type="hidden"  name="invoice_entry_product_detail_sl_feet_met[]" id="invoice_entry_product_detail_sl_feet_met'+row_cnt+'"  value="'+product_sl_feet_met+'" /> <input class="form-control" type="hidden"  name="invoice_entry_product_detail_sl_feet[]" id="invoice_entry_product_detail_sl_feet'+row_cnt+'" value="'+product_sl_feet+'"  /> <input class="form-control" type="hidden"  name="invoice_entry_product_detail_sl_feet_in[]" id="invoice_entry_product_detail_sl_feet_in'+row_cnt+'" value="'+product_sl_feet_in+'"/></td></tr>';	
				
			$("#product_detail_rws_display").append(apnd);	
			}else{ 
			
			apnd	+= '<tr><td><input type="hidden"  name="invoice_entry_product_detail_mother_child_type[]" id="invoice_entry_product_detail_mother_child_type'+row_cnt+'" value="'+mother_child_type+'" />'+product_name+'<input type="hidden"  name="invoice_entry_product_detail_product_id[]" id="invoice_entry_product_detail_product_id'+detail_id+'" value="'+product_id+'" /><input type="hidden"  name="invoice_entry_product_detail_product_type[]" id="invoice_entry_product_detail_product_type" value="'+product_type+'" /><input type="hidden"  name="invoice_entry_product_detail_quotation_detail_id[]" id="invoice_entry_product_detail_quotation_detail_id" value="'+detail_id+'" /><input type="hidden"  name="invoice_entry_product_detail_entry_type[]" id="invoice_entry_product_detail_entry_type" value="'+t_id+'" /><input type="hidden"  name="invoice_entry_product_detail_id[]" id="invoice_entry_product_detail_id'+row_cnt+'" value="" /><input type="hidden"  name="invoice_entry_product_detail_brand_id[]" id="invoice_entry_product_detail_brand_id'+row_cnt+'" value="'+product_brand_id+'" /></td>';
...
{	
	...
	document.getElementById('invoice_entry_product_detail_total'+id).value =  Math.round(product_amount);
	totalAmount();
}
...
function totalAmount(){
	...
	for(var i=1; i<=parseInt(total_row); i++) {
		...
		var toal_amount 	 	= (isNaN(total_detail_amt)?0:parseFloat(total_detail_amt));
		sum_val 				= parseFloat(sum_val)+parseFloat(toal_amount);
	}

	var gross_amount			= (isNaN(sum_val)?0:parseFloat(sum_val));
	...
	var transport_amount 		= (isNaN(transport_amount)? 0 : parseFloat(transport_amount));

	var tax_per 				= (isNaN(tax_per)? 0 : parseFloat(tax_per));

	var advance_amount 			= (isNaN(advance_amount)? 0 : parseFloat(advance_amount));

	//var tax_amount				= (tax_per*gross_amount)/100;
	var tax_amount				= (gross_amount/tax_per);
	var tax_amount 				= (isNaN(tax_amount)? 0 : parseFloat(tax_amount));

	 document.getElementById('invoice_entry_tax_amount').value	= Math.round(tax_amount);

	var net_amount				= parseFloat(transport_amount)+parseFloat(gross_amount);
	document.getElementById('invoice_entry_total_amount').value	=Math.round(net_amount);
	var net_amount				= parseFloat(net_amount)-parseFloat(advance_amount);

	var net_amount 				= (isNaN(net_amount)? 0 : parseFloat(net_amount));
...
function GetTotalLength(id){
	...
	document.getElementById('invoice_entry_product_detail_tot_length'+id).value = Math.round(total_length_val);
...
var brand_id=$("#invoice_entry_product_detail_brand_id"+id).val();
var width_inches=$("#invoice_entry_product_detail_width_inches"+id).val();
