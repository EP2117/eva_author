...
//echo $select_production_order_no;exit;
				...
				//if($_SESSION[SESS.'_session_user_branch_type']==2){
					...
					$select_det1="SELECT sum(stock_ledger_weight_tone) as qty,stock_ledger_product_id,stock_ledger_mother_child_type as mother_child_type   FROM stock_ledger
								LEFT JOIN product_con_entry_child_product_details ON product_con_entry_child_product_detail_id =stock_ledger_product_id
					 			WHERE stock_ledger_thick_ness ='".$production_order_product_detail_product_thick[$i]."' 
								AND stock_ledger_colour_id='".$production_order_product_detail_product_color_id[$i]."'
								AND product_con_entry_child_product_detail_product_brand_id='".$production_order_product_detail_product_brand_id[$i]."'
					...
					$tot_length											= 	$production_order_product_detail_inv_tot_length[$i]; //echo $tot_length;esxit;
					$child_type										    =	$production_order_product_detail_mother_child_type[$i];
	...
	stockLedger($arr_data[$b]['mother_child_type'],'out',$production_order_id,$detail_id,$produt_id,$length_feet,$length_meter,$ton_qty,$kg_qty,$produt_qty, $production_order_branch_id,  $godown_id, $production_order_date, $production_order_no,$entry_type,'2',$width_inches,$width_mm,$product_colour_id,$product_thick);
	}
					}
					if($arr_data[$b]['qty'] ==0){
					$entry_type											= 	"prodution-order";
