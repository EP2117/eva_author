<?php  
	...
	{
		$where = " WHERE quotation_entry_deleted_status =0 AND quotation_entry_financial_year = '".$_SESSION[SESS.'_session_financial_year']."' AND quotation_entry_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_entry_no']) && (!empty($_REQUEST['search_entry_no']))) {
			$where .= " AND quotation_entry_no = '".dataValidation($_REQUEST['search_entry_no'])."'";
		}
		
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND quotation_entry_branch_id = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		if(isset($_REQUEST['search_customer_id']) && (!empty($_REQUEST['search_customer_id']))) {
			$where .= " AND quotation_entry_customer_id = '".dataValidation($_REQUEST['search_customer_id'])."'";
		}
		if(isset($_REQUEST['search_color_id']) && (!empty($_REQUEST['search_color_id']))) {
			$where .= " AND quotation_entry_product_detail_product_color_id = '".dataValidation($_REQUEST['search_color_id'])."'";
		}
		
		if(isset($_REQUEST['search_thick_id']) && (!empty($_REQUEST['search_thick_id']))) {
			$where .= " AND quotation_entry_product_detail_product_thick = '".dataValidation($_REQUEST['search_thick_id'])."'";
		}
		...
		if(isset($_REQUEST['search_prodcut_id']) && (!empty($_REQUEST['search_prodcut_id']))) {
			$where .= " AND quotation_entry_product_detail_product_id = '".dataValidation($_REQUEST['search_prodcut_id'])."'";
		}
		if(isset($_REQUEST['search_raw']) && (!empty($_REQUEST['search_raw']))) {
			...
			$where .= " AND product_type IN( ".$arr_search_raw.")";
		         ...
		         $select_invoice_entry = "SELECT quotation_entry_id,product_type, quotation_entry_uniq_id, quotation_entry_no, 
								  quotation_entry_date, quotation_entry_net_amount,
								  customer_name, customer_code,product_name,product_uom_name,
								  customer_code,brand_name,	product_colour_name,
								  quotation_entry_product_detail_product_thick,quotation_entry_product_detail_product_id,
								  quotation_entry_product_detail_s_width_inches,quotation_entry_product_detail_s_width_mm,
								  quotation_entry_product_detail_sl_feet,quotation_entry_product_detail_sl_feet_met,quotation_entry_product_detail_sl_feet_in, quotation_entry_product_detail_sl_feet_mm,
								  quotation_entry_product_detail_s_weight_inches,quotation_entry_product_detail_s_weight_mm,
								  quotation_entry_product_detail_s_weight_met, quotation_entry_product_detail_rate, quotation_entry_product_detail_total, quotation_entry_product_detail_tot_length, quotation_entry_product_detail_qty 						  
								  
								  FROM quotation_entry_product_details 
								  LEFT JOIN quotation_entry ON quotation_entry_id = quotation_entry_product_detail_quotation_entry_id
								  LEFT JOIN products ON product_id = quotation_entry_product_detail_product_id	
								  LEFT JOIN brands ON brand_id = product_brand_id
								  LEFT JOIN product_uoms ON product_uom_id = product_product_uom_id
								  LEFT JOIN product_colours ON product_colour_id =  quotation_entry_product_detail_product_color_id
								  LEFT JOIN customers ON customer_id = quotation_entry_customer_id
								  LEFT JOIN cities ON city_id = customer_city_id
								  $where
								  ORDER BY quotation_entry_product_detail_product_id ASC";
