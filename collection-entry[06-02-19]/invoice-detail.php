  ...
  $select_invoice		=	"SELECT 
									...
									ON 
									 ...
									 WHERE 
										...
										IFNULL(rcv_table.rcv_amount,0) 							< invoice_entry_net_amount											
...
?>
			...
			<tr class="odd gradeX">
				...
				<input type="hidden" name="invoice_entry_net_amount" id="invoice_entry_net_amount<?php echo $record_invoice['invoice_entry_id'];?>" value="<?php echo $record_invoice['invoice_entry_net_amount']; ?>" >

				<input type="hidden" name="damage_stock_balance_amount" id="damage_stock_balance_amount<?php echo $record_invoice['invoice_entry_id'];?>" value="<?=number_format($record_invoice['invoice_entry_net_amount']-$record_invoice['received_amt'],2,'.','')?>" />
				...
				<td><?=number_format($record_invoice['invoice_entry_net_amount'],2,'.','')?></td>

				<td><?=number_format($record_invoice['received_amt'],2,'.','')?></td>

				<td><?=number_format($record_invoice['invoice_entry_net_amount']-$record_invoice['received_amt'],2,'.','')?></td>
