<?php  
	...
	{
		$where = " WHERE collection_entry_deleted_status =0 AND  collection_entry_company_id = '".$_SESSION[SESS.'_session_company_id']."' ";
		...
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND collection_entry_branch_id = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		if(isset($_REQUEST['search_customer_id']) && (!empty($_REQUEST['search_customer_id']))) {
			$where .= " AND collection_entry_customer_id = '".dataValidation($_REQUEST['search_customer_id'])."'";
		}
		       $select_invoice_entry = "SELECT 
				 							collection_entry_no, invoice_entry_no, invoice_entry_advance_amount,
											collection_entry_detail_amount, 
											DATE_FORMAT(collection_entry_date ,'%d/%m/%Y') AS collection_entry_date,
											customer_name,collection_entry_detail_disc_amount,
											DATE_FORMAT(invoice_entry_date ,'%d/%m/%Y') AS invoice_entry_date
								  FROM  collection_entry_details
								  LEFT JOIN  collection_entry ON collection_entry_detail_collection_entry_id = collection_entry_id
								  LEFT JOIN invoice_entry ON invoice_entry_id = collection_entry_detail_invoice_entry_id
								  LEFT JOIN customers ON customer_id = invoice_entry_customer_id
								  $where
								  ORDER BY collection_entry_id,collection_entry_no ASC";
