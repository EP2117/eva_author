...
function AddproductDetail(){ 
 ...
 function getLength(id){
	 var pro_id = document.getElementsByName('production_entry_product_detail_product_id[]').value;
	// var length = document.getElementById('production_entry_product_detail_tot_length'+id).value;
	 var raw_length = document.getElementById('production_entry_raw_product_detail_sl_feet1').value;
	 var total=0;
	// alert('test');
	$(".tot_length").each(function (){
		if($(this).val()!=''){
	...
	if(total>raw_length){
		alert("Must the value of total length");
	}
...
function AddRawproductDetail_original(){ 
...
function AddRawproductDetail(){ 
	...
	for (var k = 0; k < x.length; k++) {
		if (x[k].checked ==true) {
			var ord_id 	=  x[k].value;
			var detail_id 					= ord_id;
			var totalLength 				= document.getElementById('hid_total_length'+ord_id).value;
			var product_name 				= document.getElementById('raw_product_name'+ord_id).value;
			var product_id 					= document.getElementById('raw_product_id'+ord_id).value;
			...
			apnd	+= '<td><input class="form-control" type="text"  name="production_entry_raw_product_detail_sl_feet[]" id="production_entry_raw_product_detail_sl_feet'+row_cnt+'" onblur="GetRLcalFeet(1,'+row_cnt+')" value="'+totalLength+'"  /></td> <td><input class="form-control" type="text"  name="production_entry_raw_product_detail_sl_feet_mm[]" id="production_entry_raw_product_detail_sl_feet_mm'+row_cnt+'" onblur="GetRLcalFeet(4,'+row_cnt+')" onBlur="GetRTotalLength('+row_cnt+')"  value=""   /></td>';
			
			
			apnd	+= '<td><input class="form-control" type="text"  name="production_entry_raw_product_detail_ton[]" id="production_entry_raw_product_detail_ton'+row_cnt+'"  onBlur="GetRRWeightClc(1,'+row_cnt+')" value="" readonly   /></td><td><input class="form-control" type="text"  name="production_entry_raw_product_detail_kg[]" id="production_entry_raw_product_detail_kg'+row_cnt+'" readonly  value="" readonly /></td></tr>';
 ...
 function GetRRWeightClc(type,id){ 
	...
	if(type==1){
		var prod_val	= prod_ton*1000;
		$('#production_entry_raw_product_detail_kg'+id).val((prod_val).toFixed(3))	;
	}
	if(type==2){
		var prod_val	= prod_kg*0.001;
		$('#production_entry_raw_product_detail_ton'+id).val((prod_val).toFixed(5));
	}
 }
...
function GetRLcalFeet(calculation_id,id){
	...
	$.get( "../ajax-file/product-calc.php",
		...
		function(data) {
			...
			document.getElementById('production_entry_raw_product_detail_ton'+id).value 			= (eval(osf_ton)*eval(data_t[0])).toFixed(5);
