<?php  
	function quotationtypeone()
	{
		...
		if(isset($_REQUEST['search_supplier_id']) && (!empty($_REQUEST['search_supplier_id']))) {
			$where .= " AND collection_entry_customer_id = '".dataValidation($_REQUEST['search_supplier_id'])."'";
		}
		     $select_invoice_entry = "SELECT 
				 							collection_entry_no,
											DATE_FORMAT(collection_entry_date ,'%d/%m/%Y') AS collection_entry_date,
											SUM(collection_entry_detail_amount) as collection_entry_detail_amount,
											customer_name,
											SUM(collection_entry_detail_disc_amount) as collection_entry_detail_disc_amount,
											invoice_entry_no,
											DATE_FORMAT(invoice_entry_date ,'%d/%m/%Y') AS invoice_entry_date
								  FROM  collection_entry
								  LEFT JOIN collection_entry_details ON collection_entry_detail_collection_entry_id = collection_entry_id
