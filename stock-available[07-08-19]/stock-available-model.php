	...
	function stockDetailsList(){
		...
		//if(in_array('2',$_REQUEST['warehouseid'])==1){//echo 'asdf';exit;	
		 ...
		 $query	= "SELECT 
					...
					WHERE
					  $where AND stock_ledger_prd_type = '2' AND stock_ledger_mother_child_type=1
					GROUP BY 
	...
	//	echo $query;exit;
		 ...
		 $query1	= "SELECT 
					...
					ON 
						product_colour_id 								= stock_ledger_colour_id
					WHERE
					  $where AND stock_ledger_prd_type = '2' AND stock_ledger_mother_child_type=2
					GROUP BY 
						stock_ledger_prd_type,stock_ledger_product_id,stock_ledger_colour_id,stock_ledger_thick_ness";
		//echo $query1;exit;
		 ...
		 while($resultData1 = mysql_fetch_array($result1)){		 
		  ...
		  $query2	= "SELECT 
					...
					WHERE
					  $where AND stock_ledger_prd_type = '2' AND stock_ledger_mother_child_type=3
					GROUP BY 
						stock_ledger_prd_type,stock_ledger_product_id,stock_ledger_colour_id,stock_ledger_thick_ness";
		//echo $query2;exit;
	 ...
	 elseif($_REQUEST['product_status']==3){
		...
		$query	= "SELECT 
					...
					GROUP BY 
						stock_ledger_prd_type,stock_ledger_product_id,stock_ledger_colour_id,stock_ledger_thick_ness,stock_ledger_length_feet";	
				//echo $query;exit;
		 $result = mysql_query($query);
