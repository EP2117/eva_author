<?php  
	...
	{
		$where = " WHERE grn_entry_product_detail_deleted_status =0 AND grn_entry_deleted_status = 0 AND grn_entry_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_entry_no']) && (!empty($_REQUEST['search_entry_no']))) {
			$where .= " AND grn_entry_no = '".dataValidation($_REQUEST['search_entry_no'])."'";
		}
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND grn_entry_branch_id = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		if(isset($_REQUEST['search_supplier_id']) && (!empty($_REQUEST['search_supplier_id']))) {
			$where .= " AND grn_entry_production_section_id = '".dataValidation($_REQUEST['search_supplier_id'])."'";
		}
		if(isset($_REQUEST['search_color_id']) && (!empty($_REQUEST['search_color_id']))) {
			$where .= " AND grn_entry_product_detail_product_colour_id = '".dataValidation($_REQUEST['search_color_id'])."'";
		}
		if(isset($_REQUEST['search_thick_id']) && (!empty($_REQUEST['search_thick_id']))) {
			$where .= " AND grn_entry_product_detail_product_thick = '".dataValidation($_REQUEST['search_thick_id'])."'";
		}
		if(isset($_REQUEST['search_prodcut_id']) && (!empty($_REQUEST['search_prodcut_id']))) {
			$where .= " AND grn_entry_product_detail_product_id = '".dataValidation($_REQUEST['search_prodcut_id'])."'";
		}
		if(isset($_REQUEST['search_product_type']) && (!empty($_REQUEST['search_product_type']))) {
			$where .= " AND grn_entry_type_id = '".dataValidation($_REQUEST['search_product_type'])."'";
		}        
		 $select_invoice_entry = "SELECT 
				 								grn_entry_no,
												grn_entry_date,
												brand_name,
												product_colour_name,grn_entry_type_id,
												grn_entry_product_detail_product_thick,
												grn_entry_product_detail_s_width_inches,
												grn_entry_product_detail_s_width_mm,
												grn_entry_product_detail_sl_feet,
												grn_entry_product_detail_sl_feet_mm,
												...
												gin_entry_no,
												gin_entry_date
								  FROM  grn_entry_product_details
								  LEFT JOIN grn_entry ON grn_entry_id = grn_entry_product_detail_grn_entry_id
								  LEFT JOIN gin_entry ON gin_entry_id = grn_entry_product_detail_gin_entry_id
								  LEFT JOIN 
								  		products 
								  ON 
								  ...
								  LEFT JOIN product_colours ON product_colour_id =  grn_entry_product_detail_product_colour_id
								  $where
								  ORDER BY grn_entry_no ASC";
