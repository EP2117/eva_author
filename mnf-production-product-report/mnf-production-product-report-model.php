<?php  
	...
	{
		$where = " WHERE production_order_product_detail_deleted_status =0 AND production_order_deleted_status = 0 AND production_order_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_supplier_id']) && (!empty($_REQUEST['search_supplier_id']))) {
			$where .= " AND production_order_production_section_id = '".dataValidation($_REQUEST['search_supplier_id'])."'";
		}
		if(isset($_REQUEST['search_color_id']) && (!empty($_REQUEST['search_color_id']))) {
			$where .= " AND production_order_product_detail_product_color_id = '".dataValidation($_REQUEST['search_color_id'])."'";
		}
		if(isset($_REQUEST['search_thick_id']) && (!empty($_REQUEST['search_thick_id']))) {
			$where .= " AND production_order_product_detail_product_thick = '".dataValidation($_REQUEST['search_thick_id'])."'";
		}
		if(isset($_REQUEST['search_prodcut_id']) && (!empty($_REQUEST['search_prodcut_id']))) {
			$where .= " AND production_order_product_detail_product_id = '".dataValidation($_REQUEST['search_prodcut_id'])."'";
		}
		if(isset($_REQUEST['search_product_type']) && (!empty($_REQUEST['search_product_type']))) {
			$where .= " AND production_order_product_detail_entry_type = '".dataValidation($_REQUEST['search_product_type'])."'";
		}        
		 $select_invoice_entry = "SELECT 
								  ...
								  LEFT JOIN production_sections ON production_section_id = production_order_production_section_id
								  LEFT JOIN 
								  		products 
								  ON 
								  ...
								  LEFT JOIN product_colours ON product_colour_id =  production_order_product_detail_product_color_id
								  $where
