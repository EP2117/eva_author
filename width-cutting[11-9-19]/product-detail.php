		 ...
		 $query	="SELECT 
				 ...
				 LEFT JOIN
				 		(SELECT
							...
							sum(stock_ledger_product_quantity) as closing_product_quantity
						 FROM
						  	stock_ledger
						 WHERE
							...
							stock_ledger_godown_id	 	= '".$_REQUEST['godown_id']."'
							
						  GROUP BY 
						  	stock_ledger_prd_type,stock_ledger_product_id) as stk_table
				 ON
				 ...
				 WHERE 
				 		product_con_entry_child_product_detail_deleted_status	= 0 AND product_con_entry_child_product_detail_product_brand_id ='".$brand_id."' ";
...
?>		
			<tr class="odd gradeX">
				...
				<td><?php echo  $record_so_detail['closing_weight_tone']?>
				    <?php /*if($record_so_detail['product_colour_name']=='Ton'){ echo $record_so_detail['closing_weight_tone']; }else{ echo $record_so_detail['closing_product_quantity'];} */ ?></td>

			</tr>

<?php  } } else { echo 'Please select Warehouse';
 ?>
