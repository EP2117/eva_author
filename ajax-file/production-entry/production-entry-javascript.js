	...
	for (var i = 0; i < document.getElementById('so_list_form').elements.length; i++) {
		...
		if (document.getElementById('so_list_form').elements[i].checked ==true) {
			...
			var row_cnt     		= parseFloat(table.rows.length)-1;	
				...
				"<tr><td>"+grn_entry_no+"</td><td>"+grn_entry_date+"<input type='hidden'  name='production_entry_grn_entry_id' id='production_entry_grn_entry_id' value='"+grn_entry_id+"'  class='dc_id'  /><input type='hidden'  name='production_entry_type_id' id='production_entry_type_id' value='"+grn_entry_type_id+"'   /> </td><td>"+grn_entry_type+"</td> </tr>");
...
function AddproductDetail(){
	...
	for (var i = 0; i < document.getElementById('product_list_form').elements.length; i++) {
		...
		if (document.getElementById('product_list_form').elements[i].checked ==true) {
			...
			apnd	+= '<tr><td>'+product_code+'</td><td>'+product_brand_name+'</td><td>'+product_name+'<input type="hidden"  name="production_entry_product_detail_product_id[]" id="production_entry_product_detail_product_id" value="'+product_id+'" /><input type="hidden"  name="production_entry_product_detail_product_type[]" id="production_entry_product_detail_product_type" value="'+product_type+'" /><input type="hidden"  name="production_entry_product_detail_grn_detail_id[]" id="production_entry_product_detail_grn_detail_id" value="'+detail_id+'" />';
			
			apnd	+= '<td>'+product_colour_name+'<input type="hidden"  name="production_entry_product_detail_product_colour_id[]" id="production_entry_product_detail_product_colour_id" value="'+product_colour_id+'" /> <input type="hidden"  name="production_entry_product_detail_grn_entry_id[]" id="production_entry_product_detail_grn_entry_id" value="'+grn_id+'" /></td>';
			...
			apnd	+= '<td style="width:30%">'+product_uom+'<input type="hidden"  name="production_entry_product_detail_product_uom_id[]" id="production_entry_product_detail_product_uom_id'+row_cnt+'" value="'+uom_id+'"   /></td>';
			
			apnd	+= '<td style="width:40%"><input class="form-control" type="text"  name="production_entry_product_detail_qty[]" id="production_entry_product_detail_qty'+row_cnt+'"  onBlur="AccdiscountPerFind('+row_cnt+')"  /></td></tr>';
...
function GetRLcalFeet(calculation_id,id){
	...
	if(calculation_id==1){
		...
		var calc_amount 	= document.getElementById('production_entry_raw_product_detail_sl_feet_in'+id).value;	
	...
	$.get(
		...
		function(data) {
			...
			document.getElementById('production_entry_raw_product_detail_sl_feet_in'+id).value 		= data_t[1];

			document.getElementById('production_entry_raw_product_detail_sl_feet_mm'+id).value 		= data_t[2];
...
function AddDamproductDetail(){
	...
	for (var i = 0; i < document.getElementById('dam_product_list_form').elements.length; i++) {

		if (document.getElementById('dam_product_list_form').elements[i].checked ==true) {

			var ord_id 	=  document.getElementById('dam_product_list_form').elements[i].value;
...
function GetDLcalc(calculation_id,id){
 ...
 function GetWeightClc(type,id){
	...
	if(type==1){
		var prod_val	= prod_ton*1000;
		$('#production_entry_dam_product_detail_kg'+id).val(prod_val)	;
	}
	if(type==2){
		var prod_val	= prod_kg*0.001;
		$('#production_entry_dam_product_detail_ton'+id).val(prod_val);
	}
