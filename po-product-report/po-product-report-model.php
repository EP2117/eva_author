<?php  
	...
	{
		$where = " WHERE pR_deleted_status =0 AND  pR_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_entry_no']) && (!empty($_REQUEST['search_entry_no']))) {
			$where .= " AND purchase_no = '".dataValidation($_REQUEST['search_entry_no'])."'";
		}
		
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND pR_branchid = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		...
		if(isset($_REQUEST['search_brand_id']) && (!empty($_REQUEST['search_brand_id']))) {
			$where .= " AND product_brand_id = '".dataValidation($_REQUEST['search_brand_id'])."'";
		        ...
		        $select_invoice_entry = "SELECT 
								  ...
								  LEFT JOIN purchase_order ON purchaseId = pRp_purchaseId
								  LEFT JOIN products ON product_id = pRp_product_id
