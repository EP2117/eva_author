<?php 
	...
	function paymentInsertUpdate(){
		...
		mysql_query("BEGIN");	
			...
			//echo $query ;exit;
				...
				$query = "UPDATE purchase_payment SET p_branchid='".$_REQUEST['branchid']."', p_supplier_location='".$_REQUEST['supplier_location']."', p_supplier_name='".$_REQUEST['supplier_name']."', p_paymentdate='".NdateDatabaseFormat($_REQUEST['paymentdate'])."',p_paymentterm='".$_REQUEST['paymentterm']."',p_bankname='".$_REQUEST['bankname']."',p_acno='".$_REQUEST['acno']."',payment_currency_id='".$_REQUEST['payment_currency_id']."',payment_no='".$payment_no."', p_modified_by='$by', p_modified_on=NOW(),p_modified_ip='$ip' WHERE paymentId='".$_REQUEST['id']."'";
			...
			if(empty($qry)){
				...
				for($k=1;$k<=$_REQUEST['payment_count'];$k++){
					...
					update_transaction($detail_id, $entry_no, $entry_date, 'purchase-payment', $acc_dr_id, $acc_cr_id, 'D', $acc_amount_cur, $entry_remark, $branch_id,$acc_amount);	
					update_transaction($detail_id, $entry_no, $entry_date, 'purchase-payment', $acc_cr_id, $acc_dr_id, 'C', $acc_amount_cur, $entry_remark, $branch_id,$acc_amount);	
					if(empty($qry)){					
						$rollBack=true;
						break;
					}
	...
	$where	= '';
		...
		$query  = "SELECT payment_no,branch_name,DATE_FORMAT(p_paymentdate ,'%d/%m/%Y') AS p_paymentdate,supplier_name
