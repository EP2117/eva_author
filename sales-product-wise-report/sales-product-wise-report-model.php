<?php  
	...
	function quotationtypeone()
	{
		...
		if(isset($_REQUEST['search_width']) && (!empty($_REQUEST['search_width']))) {
			$where .= " AND invoice_entry_product_detail_s_width_inches = '".dataValidation($_REQUEST['search_width'])."'";
		}
		
		if(isset($_REQUEST['search_length']) && (!empty($_REQUEST['search_length']))) {
			$where .= " AND invoice_entry_product_detail_sl_feet = '".dataValidation($_REQUEST['search_length'])."'";
		}
		if(isset($_REQUEST['search_weight']) && (!empty($_REQUEST['search_weight']))) {
			$where .= " AND invoice_entry_product_detail_s_weight_inches = '".dataValidation($_REQUEST['search_weight'])."'";
		}
		
		
		
		            $select_invoice_entry = "SELECT invoice_entry_product_detail_rate,invoice_entry_product_detail_total,invoice_entry_product_detail_qty,
				   					brand_name,customer_name,invoice_entry_no,invoice_entry_date,salesman_name,invoice_entry_product_detail_product_id,
