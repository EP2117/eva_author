	...
	for (var i = 0; i < document.getElementById('so_list_form').elements.length; i++) {
		...
		if (document.getElementById('so_list_form').elements[i].checked ==true) {
			...
			var row_cnt     		= parseFloat(table.rows.length)-1;	
				...
				"<tr><td>"+gin_entry_no+"</td><td>"+gin_entry_date+"<input type='hidden'  name='grn_entry_gin_entry_id' id='grn_entry_gin_entry_id' value='"+gin_entry_id+"'  class='dc_id'  /><input type='hidden'  name='grn_entry_type_id' id='grn_entry_type_id' value='"+gin_entry_type_id+"' /></td><td>"+gin_entry_type+"</td> </tr>");
...
function AddproductDetail(){
	...
	for (var k = 0; k < x.length; k++) {
		if (x[k].checked ==true) {
			...
			apnd	+= '<td style="width:40%"><input class="form-control" type="text"  name="grn_entry_product_detail_qty[]" id="grn_entry_product_detail_qty'+row_cnt+'"  onBlur="AccdiscountPerFind('+row_cnt+')"  /></td></tr>';
...
function AddRawproductDetail(){
	...
	var x		= document.getElementsByName("gin_entry_raw_product_detail_id[]");
	for (var k = 0; k < x.length; k++) {
		if (x[k].checked ==true) {
			var ord_id						= x[k].value;
			var detail_id 					= ord_id;
