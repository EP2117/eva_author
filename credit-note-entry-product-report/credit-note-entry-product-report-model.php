<?php  
	...
	{
		$where = " WHERE credit_note_entry_deleted_status =0 AND credit_note_entry_financial_year = '".$_SESSION[SESS.'_session_financial_year']."' AND credit_note_entry_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_entry_no']) && (!empty($_REQUEST['search_entry_no']))) {
			$where .= " AND credit_note_entry_no = '".dataValidation($_REQUEST['search_entry_no'])."'";
		}
		
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND credit_note_entry_branch_id = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		if(isset($_REQUEST['search_customer_id']) && (!empty($_REQUEST['search_customer_id']))) {
			$where .= " AND credit_note_entry_customer_id = '".dataValidation($_REQUEST['search_customer_id'])."'";
		}
		if(isset($_REQUEST['search_color_id']) && (!empty($_REQUEST['search_color_id']))) {
			$where .= " AND credit_note_entry_product_detail_product_color_id = '".dataValidation($_REQUEST['search_color_id'])."'";
		}
		
		if(isset($_REQUEST['search_thick_id']) && (!empty($_REQUEST['search_thick_id']))) {
			$where .= " AND credit_note_entry_product_detail_product_thick = '".dataValidation($_REQUEST['search_thick_id'])."'";
		}
		...
		if(isset($_REQUEST['search_prodcut_id']) && (!empty($_REQUEST['search_prodcut_id']))) {
			$where .= " AND credit_note_entry_product_detail_product_id = '".dataValidation($_REQUEST['search_prodcut_id'])."'";
		}
		if(isset($_REQUEST['search_raw']) && (!empty($_REQUEST['search_raw']))) {
			$arr_search_raw =implode(',',$_REQUEST['search_raw']);
			$where .= " AND credit_note_entry_type_id IN( ".$arr_search_raw.")";
		          ...
		          $select_invoice_entry = "SELECT credit_note_entry_id,product_type, credit_note_entry_uniq_id, credit_note_entry_no, 
								  ...
								  credit_note_entry_product_detail_product_thick,credit_note_entry_product_detail_product_id,
								  credit_note_entry_product_detail_s_width_inches,credit_note_entry_product_detail_s_width_mm,
								  credit_note_entry_product_detail_sl_feet,credit_note_entry_product_detail_sl_feet_met,
								  ...
								  LEFT JOIN products ON product_id = credit_note_entry_product_detail_product_id	
								  LEFT JOIN brands ON brand_id = product_brand_id
								  LEFT JOIN product_uoms ON product_uom_id = product_product_uom_id
								  LEFT JOIN product_colours ON product_colour_id =  credit_note_entry_product_detail_color_id
								  LEFT JOIN customers ON customer_id = credit_note_entry_customer_id
								  LEFT JOIN cities ON city_id = customer_city_id
								  $where
								  ORDER BY credit_note_entry_product_detail_product_id ASC";
