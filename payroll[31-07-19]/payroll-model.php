	...
	function generatePayslip()
	{
		...
		mysql_query("BEGIN");	
		   ...
		   $select_query  = "SELECT 
								...
								IFNULL((SELECT sum(lv_leaveno) FROM emp_leave where lv_employee_id=employee_id AND lv_paymentid=1 AND DATE_FORMAT(lv_date ,'%Y-%m') ='".$yearmonth."'),0) emp_paid_leave,
								...
								IFNULL((SELECT SUM(ot_aprovalamnt) FROM emp_overtime WHERE ot_employee_id= employee_id AND DATE_FORMAT(ot_date ,'%Y-%m') ='".$yearmonth."'),0) emp_overtime
		 					FROM employees 
		  ...
		  $result = mysql_query($select_query);
			...
			while($resultData = mysql_fetch_array($result)){
				...
				$total_earnings					= $resultData['employee_basic_pay']+$resultData['emp_overtime']+$resultData['emp_allowane']+$gov_bouns+$without_bouns+$exp_bouns;
				...
				$ex_empid = mysql_fetch_array($exist_empid); 
					...
					if(empty($ex_empid['payroll_id'])){
						 ...
						 $query="INSERT INTO payroll SET payroll_branchid='".$_REQUEST['p_branchid']."',payroll_mm_yyyy='".$monthyear."',payroll_month='".$_REQUEST['p_month']."',payroll_year='".$_REQUEST['p_year']."', payroll_employee_id='".$resultData['employee_id']."', payroll_no_of_working_days='".$totDays."', payroll_no_of_worked='".$working_day."', payroll_no_of_leaves='".$resultData['emp_leave']."', payroll_basic_pay='".$resultData['employee_basic_pay']."',payroll_allowance_amount='".$resultData['emp_allowane']."',payroll_overtime_amount='".$resultData['emp_overtime']."',payroll_gov_bouns='".$gov_bouns."', payroll_without_bouns='".$without_bouns."', payroll_exp_bouns='".$exp_bouns."',  payroll_earning_amount='".$total_earnings."',payroll_advance_amount='".$resultData['emp_advance']."',payroll_loss_of_pay='".$loss_of_pay."',payroll_deduction_amount='".$total_deduct."',payroll_net_amount='".$net_salary."',payroll_branch_id='$bC', payroll_added_by='$by', payroll_added_on=NOW(), payroll_added_ip='$ip' ";
					
					}else{
						
						  $query="UPDATE payroll SET payroll_no_of_working_days='".$totDays."', payroll_no_of_worked='".$working_day."', payroll_no_of_leaves='".$resultData['emp_leave']."', payroll_basic_pay='".$resultData['employee_basic_pay']."',payroll_allowance_amount='".$resultData['emp_allowane']."',payroll_overtime_amount='".$resultData['emp_overtime']."',payroll_gov_bouns='".$gov_bouns."', payroll_without_bouns='".$without_bouns."', payroll_exp_bouns='".$exp_bouns."',payroll_earning_amount='".$total_earnings."',payroll_advance_amount='".$resultData['emp_advance']."',payroll_loss_of_pay='".$loss_of_pay."',payroll_deduction_amount='".$total_deduct."',payroll_net_amount='".$net_salary."',payroll_branch_id='$bC', payroll_modified_by='$by', payroll_modified_on=NOW(), payroll_modified_ip='$ip' where payroll_id ='".$ex_empid['payroll_id']."' ";
