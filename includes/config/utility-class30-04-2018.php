...
function Child_prod_detail($product_id){
	$select_product		=	"SELECT 
							 ...
							 WHERE 
								...
								product_con_entry_child_product_detail_id 					= '".$product_id."'					
							 ORDER BY 
...
function reduceRawMaterial($product_id,$thick,$color_id,$width_inches,$tot_len,$entry_no,$entry_date,$entry_id,$detail_id,$branch_id,$entry_type,$type_id,$ton_qty,$kg_qty,$product_detail_qty){
			...
			if($rec_product['product_type']==3 && count($get_raw)>0){
				...
				if(isset($get_raw)){
					foreach($get_raw as $get_raw_val){
						...
						stockLedger('out',$entry_id,$detail_id,$produt_id,$length_feet,$length_meter,$ton_qty,$kg_qty,$product_detail_qty, $branch_id,  	$product_con_entry_godown_id, $entry_date, $entry_no,$stock_ledger_entry_type, '2',$width_inches,$width_mm,$color_id,$thick	);
					}
				...
				if(isset($get_sec_raw)){
					...
					$stock_ledger_entry_type							= 	$entry_type;
					$product_con_entry_godown_id						= 	"2";
					stockLedger('out',$entry_id,$detail_id,$product_id,$length_feet,$length_meter,$ton_qty,$kg_qty,$product_detail_qty, $branch_id,  	$product_con_entry_godown_id, $entry_date, $entry_no,$stock_ledger_entry_type, '2',$width_inches,$width_mm,$color_id,$thick);
			}
}
...
$acc_transaction_account_id1, $acc_transaction_cord, $acc_transaction_amount, $acc_transaction_remark,  $acc_transaction_branch_id)
{
	$ip = getRealIpAddr();
	...
	$acc_transaction_number = mysql_query($transaction_select);
	...
	if($rows_transaction_no > 0) {
		...
		$update_transaction	="UPDATE acc_transaction  SET
								...
								acc_transaction_amount         = '".$acc_transaction_amount."',
		...
		$insert_transaction = "INSERT INTO acc_transaction(acc_transaction_voucher_id, acc_transaction_no, acc_transaction_date, 
												...
												VALUES ('".$acc_transaction_voucher_id."', '".$acc_transaction_no."', '".$acc_transaction_date."',
														'".$acc_transaction_type."', '".$acc_transaction_account_id."', '".$acc_transaction_account_id1."', 
														'".$acc_transaction_cord."', '".$acc_transaction_amount."', '".$acc_transaction_remark."', 
														'".$_SESSION[SESS.'_session_company_id']."', '".$acc_transaction_branch_id."','".$_SESSION[SESS.'_session_financial_year']."',
														'".$_SESSION[SESS.'_session_user_id']."', UNIX_TIMESTAMP(NOW()), '".$ip."')";   

		mysql_query($insert_transaction);
	}
} 
...
function get_products_arr($id){
			 ...
			 $select_color = "SELECT product_id, product_name FROM products WHERE product_id = '".$id."' AND product_deleted_status = 0 ORDER BY product_name ASC";
			$result_color = mysql_query($select_color);
			$arr_color    = array();
