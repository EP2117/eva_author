<?php 
	function insertUpdateReceivable(){
		...
		if(!empty($qry)){
			...
			$amount_debit_frgn = $_REQUEST['amount_debit_frgn'];
				...
				//echo $acc_amount_deb; exit;
				update_transaction($last_id, $entry_no, $entry_date, 'account-receivable',$acc_cr_id , $acc_dr_id, 'D',$acc_amount_mmk, $entry_remark, $branch_id,  $acc_amount_deb);
				
				$acc_amount_cre		= ($acc_amount!=$acc_amount_mmk)?$acc_amount:0;
				update_transaction($last_id, $entry_no, $entry_date, 'account-receivable',  $acc_dr_id, $acc_cr_id, 'C', $acc_amount_mmk, $entry_remark, $branch_id,  $acc_amount_cre);			
	...
	function listReceivable(){
	    ...
	    {
	        $where.="AND acR_branchid ='".$_REQUEST['branchid']."'";
	    }
		...
		{
		$where.="AND acR_date BETWEEN '".NdateDatabaseFormat($_REQUEST['search_from_date'])."'
		...
		$query  = "SELECT acReceivableId,branch_name,DATE_FORMAT(acR_date ,'%d/%m/%Y') AS acR_date 
					...
					WHERE acR_deleted_status=0 $where
