...
?>
<script type="text/javascript" src="<?php echo PROJECT_PATH.'/inv-damage-entry-reprot/inv-damage-entry-reprot-javascript.js'; ?>"></script>
</head>
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-head-line">Write Off Entry Report</h1>
                        ...
                        <div class="panel-body">
							<form action="index.php" method="post" id="invoice_list_form" name="invoice_list_form" >
								...
								<div class="col-lg-4">
									<div class="form-group" >
										...
										<select name="search_godown_id" id="search_godown_id" class="form-control select2" style="width:100%">
											...
											?>
													<option value="<?=$get_customer['godown_id']?>" <?php if(searchValue('search_godown_id')==$get_customer['godown_id']) { ?> selected="selected" <?php } ?>><?=$get_customer['godown_name']?></option>
											<?php
												}
											?>
										</select>
								...
								<div class="form-group" >
										...
										<select name="search_writeoff_type" id="search_writeoff_type" class="form-control select2" style="width:100%" >
													 <option value=""> - Select - </option>
													<?php
													foreach($writeof_type_arry as $key=>$val){
														if(searchValue('search_writeoff_type') == $key){ $select ='selected="selected"'; }else{ $select="";}
													?>
								...
								<table class="table table-striped table-bordered table-hover" id="product_detail_rls" >
                                    <thead  >
										 ...
										 <th rowspan="2" style="vertical-align:middle;">SNO</th>
											...
											<th rowspan="2" style="vertical-align:middle;"> WRITE OFF&nbsp;NO.</th>
											<th rowspan="2" style="vertical-align:middle;"> DATE</th>
											<th rowspan="2" style="vertical-align:middle;" nowrap="nowrap"> Damage No.</th>
											<th rowspan="2" style="vertical-align:middle;"> DATE</th>
                                    ...
                                    <tbody >
										...
										foreach($invoice_list as $get_val){
											$wr_type_id= empty($get_val['wr_type_id'])?"":$get_val['wr_type_id'];
										if($product_id!=$get_val['writeoff_entry_product_detail_product_id']){
										if($sno!=1){ ?>
										...
										?>
										<tr>
											<td colspan="18" style="text-align:center"> <?php  if($wr_type_id==2){ echo $get_val['product_con_entry_child_product_detail_code'].' - '.$get_val['product_con_entry_child_product_detail_name'];}else{echo $get_val['product_code'].' - '.$get_val['product_name'];}?></td>
										</tr>
										
										<?php } 
										if($get_val['writeoff_entry_product_detail_product_thick']==0){
										$arr_thicks='';
										}else{
										$arr_thicks=$arr_thick[number_format($get_val['writeoff_entry_product_detail_product_thick'])];
										...
										<tr>
										<td><?php echo $sno++;?></td>	
											<td><?php echo substr(('0000'.$get_val['writeoffId']),-5);?></td>
											<td><?php echo dateGeneralFormatN($get_val['wr_date']);?></td>
											<td><?php echo $get_val['damage_entry_no'];?></td>
											<td><?php echo dateGeneralFormatN($get_val['damage_entry_date']);?></td>
											<td><?php echo $get_val['brand_name'];?></td>
											
											<td><?php echo $get_val['product_colour_name'];?></td>
											<td><?php echo $arr_thicks;?></td>
											<td><?php echo $get_val['writeoff_entry_product_detail_width_inches'];?></td>
											<td><?php echo $get_val['writeoff_entry_product_detail_width_mm'];?></td>
											<td><?php echo $get_val['writeoff_entry_product_detail_length_feet'];?></td>
											<td><?php echo $get_val['writeoff_entry_product_detail_length_mm'];?></td>
											<td><?php echo $get_val['writeoff_entry_product_detail_weight_tone'];?></td>
											<td><?php echo $get_val['writeoff_entry_product_detail_weight_kg'];?></td>
											
											<td><?php echo number_format($get_val['writeoff_entry_product_detail_qty'],3,'.','');?></td>
										</tr>
										<?php 
										
										$product_id=$get_val['writeoff_entry_product_detail_product_id'];
										$subqty=$subqty+$get_val['writeoff_entry_product_detail_qty'];
										$grandqty=$grandqty+$get_val['writeoff_entry_product_detail_qty'];
										}?>
										<tr><td colspan="11" >&nbsp; </td>
											<td colspan="3" > Sub Total</td>
											<td style="text-align:right"><?php echo number_format($subqty,3,'.','');?>
