<?php  
	...
	{
		$where = " WHERE product_con_entry_child_product_detail_deleted_status =0 AND pI_deleted_status = 0 AND pI_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_prodcut_id']) && (!empty($_REQUEST['search_prodcut_id']))) {
			$where .= " AND product_con_entry_child_product_detail_product_id = '".dataValidation($_REQUEST['search_prodcut_id'])."'";
		}
		         $select_invoice_entry = "SELECT 
								  ...
								  LEFT JOIN purchase_invoice ON invoiceId = product_con_entry_child_product_detail_product_con_entry_id
