...
function AddRawproductDetail(){
	...
	for (var k = 0; k < x.length; k++) {
		if (x[k].checked ==true) {
			...
			var u = document.getElementsByName("production_entry_product_detail_product_id[]");
			//alert(u.length);
			for(g=0;g<u.length;g++){
				...
				if(p_product_thick==product_thick_ness_id && p_product_colour==product_colour_id){
					var product_sl_feet				= eval(product_sl_feet)+eval(document.getElementById('production_entry_product_detail_tot_length'+g).value);	
				}
			}
