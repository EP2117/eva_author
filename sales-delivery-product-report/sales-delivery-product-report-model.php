<?php  
	...
	{
		$where = " WHERE delivery_entry_deleted_status =0 AND delivery_entry_financial_year = '".$_SESSION[SESS.'_session_financial_year']."' AND delivery_entry_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_entry_no']) && (!empty($_REQUEST['search_entry_no']))) {
			$where .= " AND delivery_entry_no = '".dataValidation($_REQUEST['search_entry_no'])."'";
		}
		
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND delivery_entry_branch_id = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		if(isset($_REQUEST['search_customer_id']) && (!empty($_REQUEST['search_customer_id']))) {
			$where .= " AND delivery_entry_customer_id = '".dataValidation($_REQUEST['search_customer_id'])."'";
		}
		...
		if(isset($_REQUEST['search_thick_id']) && (!empty($_REQUEST['search_thick_id']))) {
			$where .= " AND delivery_entry_product_detail_product_thick = '".dataValidation($_REQUEST['search_thick_id'])."'";
		}
		...
		if(isset($_REQUEST['search_prodcut_id']) && (!empty($_REQUEST['search_prodcut_id']))) {
			$where .= " AND delivery_entry_product_detail_product_id = '".dataValidation($_REQUEST['search_prodcut_id'])."'";
		}
		...
		if(isset($_REQUEST['search_width']) && (!empty($_REQUEST['search_width']))) {
			$where .= " AND delivery_entry_product_detail_width_inches = '".dataValidation($_REQUEST['search_width'])."'";
		}
		if(isset($_REQUEST['search_length']) && (!empty($_REQUEST['search_length']))) {
			$where .= " AND delivery_entry_product_detail_tot_length = '".dataValidation($_REQUEST['search_length'])."'";
		}
		if(isset($_REQUEST['search_weight']) && (!empty($_REQUEST['search_weight']))) {
			$where .= " AND delivery_entry_product_detail_s_weight_inches = '".dataValidation($_REQUEST['search_weight'])."'";
		}
		
		
		
		           $select_invoice_entry = "SELECT delivery_entry_id,product_type, delivery_entry_uniq_id, delivery_entry_no, 
								  delivery_entry_date, 
								  customer_name, customer_code,product_name,
								  customer_code,brand_name,	product_colour_name,product_uom_name,
								  delivery_entry_product_detail_product_thick,delivery_entry_product_detail_product_id,
								  delivery_entry_product_detail_sl_feet,delivery_entry_product_detail_s_width_mm,
								  delivery_entry_product_detail_s_width_inches,delivery_entry_product_detail_s_weight_mm,
								  delivery_entry_product_detail_sl_feet_met,delivery_entry_product_detail_s_weight_inches,
								  delivery_entry_product_detail_qty
								  FROM  delivery_entry_product_details 
								  LEFT JOIN delivery_entry ON delivery_entry_id	 = delivery_entry_product_detail_delivery_entry_id
								  LEFT JOIN products ON product_id = delivery_entry_product_detail_product_id
								  LEFT JOIN brands ON brand_id = product_brand_id
								  LEFT JOIN product_uoms ON product_uom_id = product_product_uom_id
								  LEFT JOIN product_colours ON product_colour_id =  product_product_category_id
								  LEFT JOIN customers ON customer_id = delivery_entry_customer_id
								  LEFT JOIN cities ON city_id = customer_city_id
								  $where
								  ORDER BY delivery_entry_product_detail_product_id ASC";
