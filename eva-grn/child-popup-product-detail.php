	...
	$m_id = $_GET['m_id'];
   	if($m_id == '') {
		  ...
		  $inv_qry	= "	SELECT
						...
						LEFT JOIN 
							(SELECT 
								grn_child_product_detail_length_feet AS received_qty,
								grn_child_product_detail_inv_detail_id
							FROM 
								grn_child_product_details
							WHERE 
								grn_child_product_detail_deleted_status 				= 0  				
							GROUP BY 
								grn_child_product_detail_inv_detail_id) rcv_table
						ON	
							grn_child_product_detail_inv_detail_id = product_con_entry_child_product_detail_id
							
				   	WHERE
					   		invoiceId 						= '".$po_id."'																		AND
							pI_deleted_status				= 0																					AND
							IFNULL(rcv_table.received_qty,0) 			< product_con_entry_child_product_detail_length_feet					AND 
...
?>
			<tr class="odd gradeX">
				...
				<input type="hidden" name="product_mother_child_type" id="product_mother_child_type<?php echo $record_so_detail['product_con_entry_child_product_detail_id'];?>" value="2" >
				</td>
