<?php  
	...
	{
		$where = " WHERE gin_entry_product_detail_deleted_status =0 AND gin_entry_deleted_status = 0 AND gin_entry_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_entry_no']) && (!empty($_REQUEST['search_entry_no']))) {
			$where .= " AND gin_entry_no = '".dataValidation($_REQUEST['search_entry_no'])."'";
		}
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND gin_entry_branch_id = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		if(isset($_REQUEST['search_supplier_id']) && (!empty($_REQUEST['search_supplier_id']))) {
			$where .= " AND gin_entry_production_section_id = '".dataValidation($_REQUEST['search_supplier_id'])."'";
		}
		if(isset($_REQUEST['search_color_id']) && (!empty($_REQUEST['search_color_id']))) {
			$where .= " AND gin_entry_product_detail_product_color_id = '".dataValidation($_REQUEST['search_color_id'])."'";
		}
		if(isset($_REQUEST['search_thick_id']) && (!empty($_REQUEST['search_thick_id']))) {
			$where .= " AND gin_entry_product_detail_product_thick = '".dataValidation($_REQUEST['search_thick_id'])."'";
		}
		if(isset($_REQUEST['search_prodcut_id']) && (!empty($_REQUEST['search_prodcut_id']))) {
			$where .= " AND gin_entry_product_detail_product_id = '".dataValidation($_REQUEST['search_prodcut_id'])."'";
		}
		if(isset($_REQUEST['search_product_type']) && (!empty($_REQUEST['search_product_type']))) {
			$where .= " AND gin_entry_type_id = '".dataValidation($_REQUEST['search_product_type'])."'";
		}        
		 $select_invoice_entry = "SELECT 
				 								gin_entry_no,
												gin_entry_date,
												brand_name,
												product_colour_name,gin_entry_type_id,
												gin_entry_product_detail_product_thick,
												gin_entry_product_detail_s_width_inches,
												gin_entry_product_detail_s_width_mm,
												gin_entry_product_detail_sl_feet,
												gin_entry_product_detail_sl_feet_mm,
												gin_entry_product_detail_s_weight_inches,
												gin_entry_product_detail_s_weight_mm,
												gin_entry_product_detail_qty,
												product_code,
												product_name,
												production_order_no,
												production_order_date
								  FROM  gin_entry_product_details
								  LEFT JOIN gin_entry ON gin_entry_id = gin_entry_product_detail_gin_entry_id
								  LEFT JOIN production_order ON production_order_id = gin_entry_product_detail_po_entry_id
								  LEFT JOIN 
								  		products 
								  ON 
								  ...
								  LEFT JOIN product_colours ON product_colour_id =  gin_entry_product_detail_product_color_id
								  $where
								  ORDER BY gin_entry_no ASC";
