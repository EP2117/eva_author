	...
	function quotationReport()
	{
		...
		if(isset($_REQUEST['search_customer_id']) && (!empty($_REQUEST['search_customer_id']))) {
			$where .= " AND delivery_entry_customer_id = '".dataValidation($_REQUEST['search_customer_id'])."'";
		     ...
		     $select_invoice_entry = "SELECT delivery_entry_id, delivery_entry_uniq_id, delivery_entry_no, 
								  delivery_entry_date, SUM(delivery_entry_product_detail_qty) as delivery_entry_product_detail_qty,
								  customer_name, customer_code,
								  customer_code
								  FROM delivery_entry
								  LEFT JOIN delivery_entry_product_details ON delivery_entry_id = delivery_entry_product_detail_delivery_entry_id
								  LEFT JOIN customers ON customer_id = delivery_entry_customer_id
								  LEFT JOIN cities ON city_id = customer_city_id
								  $where
								  GROUP BY delivery_entry_product_detail_delivery_entry_id
								  ORDER BY delivery_entry_date ASC, delivery_entry_no ASC";
