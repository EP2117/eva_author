...
$select_collection_entry_no						= "SELECT 
														MAX(SUBSTRING(collection_entry_no,3,7)) AS maxval
														   ...
														   WHERE 
																...
																//echo $select_collection_entry_no	;exit;
		...
		if($maxval > 0) {
			...
			//echo $collection_entry_no;exit;
		} else {
			$collection_entry_no 							=substr(('00001'.++$maxval),-5);
			//echo $collection_entry_no;exit;
		}
		return $collection_entry_no;
		
}
	function insertPayment(){
	
	$select_branch	= "SELECT branch_prefix FROM branches WHERE  branch_id = '".$_POST['collection_entry_branch_id']."' ";
		//echo $select_branch;exit;
		$result_branch = mysql_query($select_branch);
		$res = mysql_fetch_array($result_branch); 
        
		$collection_entry_no							= 'DR'.GetAutoNo($_POST['collection_entry_branch_id']);
		//$collection_entry_no							= empty($collection_entry_no1)?'DR'.GetAutoNo($_POST['collection_entry_no']):$collection_entry_no1;
		$collection_entry_branch_id                   	= trim($_POST['collection_entry_branch_id']);
		...
		for($po_index = 0; $po_index < count($collection_entry_detail_invoice_entry_id); $po_index++) {
			...
			if($detail_request_fields) {
				...
				$discount_amt	= $collection_entry_detail_disc_amount[$po_index];
				...
				//echo $acc_amount."-".$acc_amount_mmk; exit;
				
				update_transaction($entry_id, $entry_no, $entry_date, 'collection', $acc_dr_id, $acc_cr_id, 'D', $acc_amount, $entry_remark, $branch_id,$acc_amount_mmk);	
				$dr_currency_amt	= getCurrencyAmt($acc_dr_id,$collection_entry_date);
				$acc_amount_mmk	= $acc_amount/$dr_currency_amt;
				$acc_amount_mmk	= ($acc_amount_mmk==$acc_amount)?0:$acc_amount_mmk;
				update_transaction($entry_id, $entry_no, $entry_date, 'collection', $acc_cr_id, $acc_dr_id, 'C', $acc_amount, $entry_remark, $branch_id,$acc_amount_mmk);			
				
				if($discount_amt>0){
				$acc_cr_id		= $cash_id;	
				$currency_amt	= getCurrencyAmt($acc_cr_id, $collection_entry_date);
				$acc_amount_mmk	= $discount_amt/$currency_amt;
				$acc_amount_mmk	= ($acc_amount_mmk==$discount_amt)?0:$acc_amount_mmk;
				
				//echo $acc_amount."-".$acc_amount_mmk; exit;
				
				update_transaction($entry_id, $entry_no, $entry_date, 'collection-discount', $acc_dr_id, $acc_cr_id, 'D', $discount_amt, $entry_remark, $branch_id,$acc_amount_mmk);	
				$dr_currency_amt	= getCurrencyAmt($acc_dr_id,$collection_entry_date);
				$acc_amount_mmk	= $discount_amt/$dr_currency_amt;
				$acc_amount_mmk	= ($acc_amount_mmk==$discount_amt)?0:$acc_amount_mmk;
				update_transaction($entry_id, $entry_no, $entry_date, 'collection-discount', $acc_cr_id, $acc_dr_id, 'C', $discount_amt, $entry_remark, $branch_id,$acc_amount_mmk);	
				}
	...
	function updateQuotation(){
		...
		for($po_index = 0; $po_index < count($collection_entry_detail_invoice_amount); $po_index++) {
			...
			if($detail_request_fields) {
				...
				update_transaction($entry_id, $entry_no, $entry_date, 'collection-discount', $acc_dr_id, $acc_cr_id, 'D', $acc_amount, $entry_remark, $branch_id,$acc_amount_mmk);	
				$dr_currency_amt	= getCurrencyAmt($acc_dr_id,$collection_entry_date);
				$acc_amount_mmk	= $acc_amount/$dr_currency_amt;
				$acc_amount_mmk	= ($acc_amount_mmk==$acc_amount)?0:$acc_amount_mmk;
				update_transaction($entry_id, $entry_no, $entry_date, 'collection-discount', $acc_cr_id, $acc_dr_id, 'C', $acc_amount, $entry_remark, $branch_id,$acc_amount_mmk);	
				}
...

   {
	...
	$delete="UPDATE collection_entry_details SET collection_entry_detail_deleted_status = 1 

						WHERE collection_entry_detail_id = ".$product_detail_id."";
			//echo $delete;exit;
			mysql_query($delete);
   ...
   function ReceivedfrmQty($detail_id){
	...
	for($i=0; $i < $count; $i++) {
		...
		$result_rec_qty			= mysql_query($select_rec_qty); //echo $select_rec_qty; exit;
		
		/*$delete_invoice = "SELECT 
										invoice_entry_id
									FROM 
										invoice_entry	
									WHERE 
										invoice_entry_quotation_entry_id 			= 0  					AND
										invoice_entry_id		= '".$detail_id."'					
									GROUP BY 
										invoice_entry_id";
								//echo $delete_invoice;exit;		
				$result_rec_qty			= mysql_query($delete_invoice);	*/
		...
		while($record_collection_entry = mysql_fetch_array($result_rec_qty)){ 
				...
				DeleteAccountTrasaction($record_collection_entry['collection_entry_detail_id'], 'collection-discount');
		}
	}
