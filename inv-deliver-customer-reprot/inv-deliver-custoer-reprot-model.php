<?php  
	...
	{
		$where = " WHERE delivery_customer_deleted_status =0 AND delivery_customer_financial_year = '".$_SESSION[SESS.'_session_financial_year']."' AND delivery_customer_company_id = '".$_SESSION[SESS.'_session_company_id']."'";
		...
		if(isset($_REQUEST['search_entry_no']) && (!empty($_REQUEST['search_entry_no']))) {
			$where .= " AND delivery_customer_no = '".dataValidation($_REQUEST['search_entry_no'])."'";
		}
		
		if(isset($_REQUEST['search_branch_id']) && (!empty($_REQUEST['search_branch_id']))) {
			$where .= " AND delivery_customer_branch_id = '".dataValidation($_REQUEST['search_branch_id'])."'";
		}
		if(isset($_REQUEST['search_customer_id']) && (!empty($_REQUEST['search_customer_id']))) {
			$where .= " AND delivery_customer_customer_id = '".dataValidation($_REQUEST['search_customer_id'])."'";
		}
		...
		if(isset($_REQUEST['search_thick_id']) && (!empty($_REQUEST['search_thick_id']))) {
			$where .= " AND delivery_customer_product_detail_product_thick = '".dataValidation($_REQUEST['search_thick_id'])."'";
		}
		if(isset($_REQUEST['search_product_id']) && (!empty($_REQUEST['search_product_id']))) {
			$where .= " AND delivery_customer_product_detail_product_id = '".dataValidation($_REQUEST['search_product_id'])."'";
		}
		
		if(isset($_REQUEST['search_width']) && (!empty($_REQUEST['search_width']))) {
			$where .= " AND delivery_customer_product_detail_width_inches = '".dataValidation($_REQUEST['search_width'])."'";
		}
		
		
		
		
		            $select_invoice_entry = "SELECT delivery_customer_id,product_type, delivery_customer_uniq_id, delivery_customer_no	, 
								  ...
								  delivery_customer_product_detail_product_thick,delivery_customer_product_detail_product_id,
								  delivery_customer_product_detail_sl_feet,delivery_customer_product_detail_s_width_mm,
								  delivery_customer_product_detail_s_width_inches,delivery_customer_product_detail_s_weight_mm,
								  delivery_customer_product_detail_sl_feet_met,delivery_customer_product_detail_s_weight_inches,
								  delivery_customer_product_detail_qty,invoice_entry_no,invoice_entry_date
								  FROM  delivery_customer_product_details 
								  LEFT JOIN delivery_customer ON delivery_customer_id = delivery_customer_product_detail_delivery_customer_id
								  LEFT JOIN products ON product_id = delivery_customer_product_detail_product_id
								  LEFT JOIN invoice_entry ON invoice_entry_id = delivery_customer_product_detail_invoice_entry_id
								  LEFT JOIN brands ON brand_id = product_brand_id
								  LEFT JOIN product_uoms ON product_uom_id = product_product_uom_id
								  LEFT JOIN product_colours ON product_colour_id =  delivery_customer_product_detail_color_id
								  LEFT JOIN customers ON customer_id = delivery_customer_customer_id	
								  LEFT JOIN cities ON city_id = customer_city_id
								  $where
								  ORDER BY delivery_customer_product_detail_product_id ASC";
