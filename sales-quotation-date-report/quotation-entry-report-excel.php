...
$objPHPExcel->setActiveSheetIndex(0)->getStyle('A'.$row.":".'E'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
$total_grand		= 0;
	...
	if($customer_id !=$value['quotation_entry_date'])
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('B'.$row, $value['quotation_entry_no'])
					->setCellValue('C'.$row, dateGeneralFormatN($value['quotation_entry_date']))
					->setCellValue('D'.$row, $value['customer_name'].'-'.$value['customer_code'])
			
					->setCellValue('E'.$row, $value['quotation_entry_net_amount']);
		...
		$total_amt += $value['quotation_entry_net_amount'];
		$total_grand += $value['quotation_entry_net_amount'];
		$objPHPExcel->getActiveSheet()->getStyle('E'.$row)->getNumberFormat()->setFormatCode('0.00');
	...
	$objPHPExcel->setActiveSheetIndex(0)->getStyle('A'.$row.":".'D'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
	...
	$objPHPExcel->setActiveSheetIndex(0)
					->setCellValue('E'.$row, number_format($total_grand,2,'.',''));
	$objPHPExcel->getActiveSheet()->getStyle('A'.$row.":".'E'.$row)->applyFromArray($styleArray2);
	$objPHPExcel->getActiveSheet()->getStyle('A'.$row.":".'E'.$row)->getFont()->setBold(true);
	$objPHPExcel->getActiveSheet()->getStyle('E'.$row)->getNumberFormat()->setFormatCode('0.00');
