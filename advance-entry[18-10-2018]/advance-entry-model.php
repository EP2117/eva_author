...
function GetAutoNo(){
	...
	function insertQuotation(){
		...
		$insert_advance_entry 							= sprintf("INSERT INTO advance_entry  (advance_entry_uniq_id, advance_entry_date,
																							  ...
																							  advance_entry_dis_per,advance_entry_dis_amount) 
																			VALUES 	 		 ('%s', '%s', 
																							  '%d', '%s', 
																							  '%f', '%f', 
																							  '%f', '%f', 
			...
			if($detail_request_fields) {
				...
				if($_SESSION[SESS.'_session_user_branch_type']==1){
					if($advance_entry_product_detail_entry_type[$i]=='4'){
							...
							stockLedger('out',$advance_entry_id,$detail_id,$produt_id,$length_feet,$length_meter,$ton_qty,$kg_qty,$produt_qty, $advance_entry_branch_id,  $godown_id, $advance_entry_date, $advance_entry_no,$stock_ledger_entry_type,'1');
					}
					else{
					$produt_id											=	$advance_entry_product_detail_product_id[$i];
