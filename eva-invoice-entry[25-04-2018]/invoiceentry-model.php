<?php 
	...
	function invoiceInsertUpdate(){
		...
		mysql_query("BEGIN");	
			...
			if(empty($qry)){
				...
				for($k=1;$k<=count($_REQUEST['arr_count']);$k++){ //echo $k;exit;
					
				   if(empty($_REQUEST['pid_'.$k])){
				
						$query ="INSERT INTO purchase_invoice_products SET piP_invoiceId='".$last_id."',piP_product_id='".$_REQUEST['productid'.$k]."',piP_po_qty='".str_replace(',','',$_REQUEST['po_qty_'.$k])."',piP_rate='".str_replace(',','',$_REQUEST['rate_'.$k])."', piP_frgn_rate='".str_replace(',','',$_REQUEST['frgn_rate_'.$k])."',piP_po_ton='".str_replace(',','',$_REQUEST['prd_ton_'.$k])."',piP_po_kg='".str_replace(',','',$_REQUEST['prd_kg_'.$k])."',piP_po_loss_ton='".str_replace(',','',$_REQUEST['prd_loss_ton_'.$k])."',piP_po_loss_kg='".str_replace(',','',$_REQUEST['prd_loss_kg_'.$k])."',piP_po_total_ton='".str_replace(',','',$_REQUEST['prd_total_ton_'.$k])."',piP_po_total_kg='".str_replace(',','',$_REQUEST['prd_total_kg_'.$k])."',piP_po_loss_amount='".str_replace(',','',$_REQUEST['prd_loss_amount_'.$k])."', piP_dispercent='".str_replace(',','',$_REQUEST['dispercent_'.$k])."',piP_disamnt='".str_replace(',','',$_REQUEST['disamnt_'.$k])."', piP_disamnt_cur='".str_replace(',','',$_REQUEST['disamnt_cur_'.$k])."', piP_total='".str_replace(',','',$_REQUEST['total_'.$k])."',piP_total_amt='".str_replace(',','',$_REQUEST['total_amt_'.$k])."',piP_frgntotal='".str_replace(',','',$_REQUEST['receive_qty_'.$k])."',piP_po_id='".$_REQUEST['purchaseid']."',piP_po_detail_id='".$_REQUEST['po_detail_id_'.$k]."'";
				
					 }else{
				  $K=$k;
					    $query ="UPDATE purchase_invoice_products SET piP_invoiceId='".$last_id."',piP_product_id='".$_REQUEST['productid'.$K]."',piP_po_qty='".str_replace(',','',$_REQUEST['po_qty_'.$K])."',piP_rate='".str_replace(',','',$_REQUEST['rate_'.$K])."', piP_frgn_rate='".str_replace(',','',$_REQUEST['frgn_rate_'.$K])."',piP_po_ton='".str_replace(',','',$_REQUEST['prd_ton_'.$K])."',piP_po_kg='".str_replace(',','',$_REQUEST['prd_kg_'.$K])."',piP_po_loss_ton='".str_replace(',','',$_REQUEST['prd_loss_ton_'.$K])."',piP_po_loss_kg='".str_replace(',','',$_REQUEST['prd_loss_kg_'.$K])."',piP_po_total_ton='".str_replace(',','',$_REQUEST['prd_total_ton_'.$K])."',piP_po_total_kg='".str_replace(',','',$_REQUEST['prd_total_kg_'.$K])."',piP_po_loss_amount='".str_replace(',','',$_REQUEST['prd_loss_amount_'.$K])."', piP_dispercent='".str_replace(',','',$_REQUEST['dispercent_'.$K])."',piP_disamnt='".str_replace(',','',$_REQUEST['disamnt_'.$K])."', piP_disamnt_cur='".str_replace(',','',$_REQUEST['disamnt_cur_'.$K])."',piP_total='".str_replace(',','',$_REQUEST['total_'.$K])."',piP_total_amt='".str_replace(',','',$_REQUEST['total_amt_'.$K])."',piP_frgntotal='".str_replace(',','',$_REQUEST['receive_qty_'.$K])."' WHERE piP_invoiceId='".$last_id."' AND invoiceProductId='".$_REQUEST['pid_'.$K]."'";
		...
		$product_con_entry_amount_by_mmk 		= $_POST['product_con_entry_amount_by_mmk'];
			...
			if(empty($rollBack)){	
				...
				$acc_amount		= $_REQUEST['net_total'];
				$entry_remark	= $_REQUEST['remark'];
				update_transaction($last_id, $entry_no, $entry_date, 'purchase-invoice', $acc_dr_id, $acc_cr_id, 'D', $acc_amount, $entry_remark, $branch_id);	
				update_transaction($last_id, $entry_no, $entry_date, 'purchase-invoice', $acc_cr_id, $acc_dr_id, 'C', $acc_amount, $entry_remark, $branch_id);		
	...
	function listinvoice(){
	 ...
	 function editInvoicedetail($id){
			  $query  = "SELECT *,supplier_name,supplier_location,DATE_FORMAT(pI_invoice_date ,'%d/%m/%Y') AS pI_invoice_date,DATE_FORMAT(pR_purchase_date ,'%d/%m/%Y') AS pR_purchase_date,pI_purchaseId,pR_advanceAmnt,pR_advance_amount
				    FROM purchase_invoice 
					LEFT JOIN purchase_order ON purchaseId = pI_purchaseId 
					LEFT JOIN suppliers ON pR_supplier_id = supplier_id
					WHERE 
